...
### The ten PropertyTypes

`string` · `number` · `money` · `boolean` · `timestamp` · `date` · `json` ·
`enum` · `set<enum>` · `file`.
`timestamp` is an ISO-8601 UTC string (never an epoch int); `date` is a bare
ISO calendar date (`YYYY-MM-DD`, no time, no zone — filters and formatting
compare calendar days, and `$now`/`:today` bounds resolve to dates); `money`
is INTEGER minor units (øre, cents) in an `INTEGER` column and MUST declare a
`currency` (ISO-4217) — a fractional amount is refused, never rounded, and views
render it as an amount without a `format` hint; `enum` is a string narrowed to
a `values` set; `set<enum>` is a closed SUBSET of `values`, written as an array
and stored as canonical JSON text in declaration order (so the whole set works
as a UNIQUE-key component and an equality operand), filtered with
`contains` / `has_any` / `has_all`; `file` holds a `_files` id as TEXT.
...
An ObjectType whose rows are periods *in force from a date until another
date* (prices, standing orders, ratios) declares `temporal` instead of
hand-writing the in-effect clause and the set/end handlers:
...
Rows are half-open periods `[from, to)` per `key`; `to` is a nullable
`string`/`timestamp` property and `NULL` means still open — exactly one open
period per key, enforced by a partial `UNIQUE` index the declaration creates
(`WHERE to IS NULL`, `NULL` key members collapse to one series). The
declaration generates two verbs, present in every surface with conformance
fixtures and never exported into `actions/`:

- **`<type>.set`** takes the row's properties (`from` required, `to`
  optional): closes the key's open period *at* the new `from` (abutting),
  inserts the new row, returns it plus `closed_id`. Any intersecting period —
  open or already closed — is refused: `VALIDATION` /
  `details: { code: 'temporal_overlap', conflicting_id }`; `to <= from` is
  `temporal_empty_period`. A period the caller may not read is never named:
  the refusal omits `conflicting_id` and its window, and `closed_id` is left
  out (it is `null` only when nothing was closed).
- **`<type>.end`** takes `to` plus either `id` or the key: stamps the open
  period's `to`. `temporal_end_before_start`, `temporal_not_open` (the `id`
  names a closed row), `NOT_FOUND` / `temporal_no_open_period`.

Reads take **`as_of`** (`objects get|list|navigate … --as-of <date>`, REST
`?as_of=`, MCP `as_of`; a date, an ISO date-time, or `$now`) and every filter
map takes the **`$as_of`** token — a ViewSpec `source.filter`, a handler
`where`, a REST `?where=` — compiling to `from <= D AND (to IS NULL OR to > D)`.
A non-temporal target refuses both with `as_of_not_temporal`.

A hand-declared action with a generated verb's name replaces it (keep your own
`price.set` with domain guards while adopting the declaration); the other verb
stays generated. Full contract:
[declaring-objecttypes.md](../canonify/resources/declaring-objecttypes.md#effective-dated-objects--temporal).
...
- **input** — a flat record of field → descriptor. `type` is `string` |
  `number` | `money` | `boolean` | `date` | `json` | `set<enum>` (no
  `timestamp`/`enum`/`file` at the input layer; an enum input is `enum: [...]`
  over a `string`; a `set<enum>` input REQUIRES `enum: [...]` and takes an
  array of distinct members; a `money` input is integer minor units and
  accepts `min`/`max`; a `date` input is `YYYY-MM-DD`). Optional flags:
...
`update` · `delete` · `assert` · `transition` · `call` · `for_each`. Each may
carry a `when` guard predicate (mandatory on `assert`). Optional top-level
`returns` block and `on_fail` compensator. `read_many` binds a bounded row set
for a following `for_each`; the loop body contains every other step kind but
cannot nest a `for_each`. `call` runs a fragment declared once with
`fragments.declare` (a bundle's `fragments/`), binding its inputs from `with`
and exposing its `returns` as `$<as>.<field>` — see the handler patterns
resource, Recipe 12.
...
remain null-tolerant so `present`/`absent` guards can inspect these bindings.

Narrowing is flow-sensitive: a nullable binding counts as present for exactly
the region in which the DSL can prove it. A `when: { ref: "$guest.id",
present: true }` narrows `$guest` for that guarded step's own maps; the same
`when` on a `for_each` narrows it for the whole loop BODY, because the guard
holds on every iteration; and an `assert` carrying the guard narrows it for
every following step to the end of the enclosing block, because a failed
assert aborts the handler. `present: true` and `neq: null` are the two
spellings of the proof, and an `all_of` guard narrows every alias its
top-level members prove. A leaf inside an `any_of` proves nothing — a
different disjunct may be the one that held — and an unguarded step after a
skipped read still sees the binding as nullable.
...
`limit`; the property must be a declared number, money, string, timestamp, or
date. For
...
If an omitted-limit `read_many` matches more than 500 rows, the action fails
with a `VALIDATION` envelope whose structured details include
`{ "code": "row_cap_exceeded", "step": <index>, "object": "<type>",
"as": "<binding>", "cap": 500 }`; the message repeats the object, step, and
cap. The set is not bound, `for_each` does not start, and the entire action
transaction (including writes before the read) rolls back. This is a loud
...
`gte` · `lt` · `lte` · `in` · `present` · `absent` · `contains` · `has_any` ·
`has_all`. `read_optional` binds `null` (not `NOT_FOUND`)
...
`all_of` is the conjunction and is **guard-only** — `when` and `assert` only,
never a `where` map, whose columns already AND. Its members are leaves or
`any_of` groups (conjunctive normal form, one level deep); `all_of` inside
`all_of` is `predicate_all_of_flat`, `all_of` inside `any_of` is
`predicate_any_of_flat`, and there is still no `not`. Use it instead of
chaining `assert` steps when the conditions are one decision, and always
instead of count arithmetic such as
`eq: { "op": "-", "left": 2, "right": "$locked.count" }` — one miscounted
operand there makes a guard pass silently.
...
declared `type: "json"` or `type: "set<enum>"`, and its operand must resolve to a scalar (`string`,
`number`, `boolean`, `enum`, `money`, `date`, or `timestamp`). A raw JSON cell read from
...
`has_any` / `has_all` are the set operators for a `set<enum>` property only
(`set_operator_requires_set_enum` otherwise): each takes an array of flat
values (literals or refs, never expressions) and holds when the stored set
shares at least one member (`has_any`) or carries every member (`has_all`).
For example: `when: { "ref": "$item.dietary_variant", "has_all": ["vegan", "gluten_free"] }`.
A whole set is also an equality operand: `where: { "dietary_variant": "$input.dietary_variant" }`
matches the row whose set has exactly those members, in any input order — the
platform canonicalizes both sides — which is what makes a `set<enum>` usable as
a UNIQUE-key component without enumerating every subset as a literal.
...
`lt` · `lte` · `neq` · `in` · `present` · `absent` · `contains` · `has_any` ·
`has_all`; literal `in`
...
bound parameters; an empty list compiles to `1=0`. `contains`, `has_any` and
`has_all` compile to `EXISTS` / `COUNT(DISTINCT …)` queries over SQLite's
`json_each`, so a JSON-array (`json` or `set<enum>`) property is required; an
empty `has_any` compiles to `1=0` and an empty `has_all` to `1=1`. For example,
a Pax-style weekday filter uses:
...
Declare Pax-style `weekdays` columns as `type: "set<enum>"` with the weekday
`values` (or `type: "json"` for an open array), never `type: "string"`;
...
same-tx rows. `sum` and `avg` require numeric (`number` or `money`) columns;
`min` and `max` also accept string, timestamp and date columns and bind a
result with the aggregated column's type. Arithmetic over a `money` value stays
in minor units and a money column refuses a fractional result — round
explicitly (`round`/`floor`/`ceil`) where your rounding policy says so. `set`/`values` may hold a flat Value expression — arithmetic
...
Operands nest: any `left`/`right` position may hold another expression, up to
five levels (`value_expr_too_deep` beyond that), so
`round($p.unit * $p.qty * (1 - $p.discount / 100), 2)` is one `set` instead of
three writes through a scratch row. `date_add`/`date_sub` `right` stays a
literal duration token, and `has_any`/`has_all` members stay flat.
All recipes, plus the `on_fail`
...
- **Two principal populations.** External signers (a non-member kind, reached by
  ...
  (`user`/`agent`) authenticate by their session. The external-signer actions
  admit the `external` kind at runtime; every other platform action refuses it,
  whatever a hosted app's access manifest grants.
  Each external-only handler still carries its own kind-guard against members.
