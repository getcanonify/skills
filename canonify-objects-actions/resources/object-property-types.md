...
- `date` — a bare ISO calendar date (`YYYY-MM-DD`) in a TEXT or `DATE` column
  (a `DATE` DDL column infers as `date`; `DATETIME`/`TIMESTAMP` infer as
  `timestamp`). Range filters, `$now±…` and `:today`-style tokens resolve to
  calendar dates, `date_add`/`date_sub` on a date returns a date, and
  `format: "date"` accepts it.
- `money` — INTEGER minor units (øre, cents, fils) in an `INTEGER` column;
  MUST declare `currency` (ISO-4217, three uppercase letters) — the validator
  rejects a currency-less money property. Input, storage and every typed read
  are the integer; a fractional value is refused with `VALIDATION`, never
  rounded. Views render it as an amount in the currency's own precision with
  no `format` hint; a range filter or form takes major units from a person and
  sends minor units. It is accepted wherever `number` is (`sum`/`avg`,
  lookups, measures, `+ - * /`, rollups).
- `json` — a TEXT column holding serialized JSON.
- `enum` — a string column narrowed to a fixed `values` set (see below).
- `set<enum>` — a closed SUBSET of `values`: written as an array of distinct
  members (auto-CRUD and custom actions alike), stored as canonical JSON text
  in declaration order (`["vegan","gluten_free"]`), read back as an array on
  every typed surface. Canonical storage makes the whole set a UNIQUE-key
  component and an equality operand; membership filters are `contains`,
  `has_any` and `has_all`. `values`, `valueLabels` and `valueColors` apply
  exactly as for `enum`.
...
flag (`is_*`, `has_*`, `at_risk`, …) should be reviewed during authoring and
changed to `boolean` when it represents a flag.
