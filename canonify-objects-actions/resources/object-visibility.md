...
status logic in `packages/canon/src/runtime-reads.ts` / `runtime-invoke.ts`.
...
2. **Granted mutations are flagged in the audit trail.** A granted invoke that
   ...
   `'success'` (`runtime-invoke.ts`). Granted reads use the reads-denials-only policy
   ...
   audited, with `status='sensitive'` instead of `'success'` (`runtime-invoke.ts`),
