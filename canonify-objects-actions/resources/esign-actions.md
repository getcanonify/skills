...
- `packages/canon/src/actions/signature-request/` — `create` / `send` / `view` / `sign` / `decline` inputs + guards.
- `packages/canon/src/actions/signature/` — `verify` / `certificate`.
- `packages/canon/src/actions/signature-envelope/` — `create` / `send` / `countersign` / `download` / `my-pending-countersigns` / `decline` / `void`, and `complete.ts` (the automatic completion step).
- `packages/canon/src/actions/member-signature/` — `set` / `get`.
- `packages/canon/src/object-types/signature-request.ts` / `signature-envelope.ts` — the status enums + `parts_frozen_when`.
