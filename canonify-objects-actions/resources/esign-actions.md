...
The e-signature actions are **platform primitives** — registered by the
e-signature catalog plugin the platform mounts into every org
(`packages/esign/src/plugin.ts`), not declared with `actions.declare`. You **invoke** them like any other action (CLI / REST /
...
platform's signing-protocol operations doc, not this skill. This skill
covers *what action to call and with what input*; how a signer's browser
redeems a link is platform behaviour you get without declaring anything.

Every shape below is grounded in the action's effect/Schema input in
`packages/esign/src/actions/{signature-request,signature,signature-envelope,member-signature}/`.
...
- **`external`** (a non-member signer reached by an emailed magic link). These
  actions admit the `external` kind at runtime (discovery's `allowed_principals`
  lists member kinds only), and every other platform action refuses it before
  its handler runs — a hosted app's access manifest cannot widen that. Each external-only handler also
  carries its **own** `principal.kind === 'external'` guard, which is what
  refuses a member caller; prove the signer path with an `external` principal
  and the guard with a member in the scenario.
  External actions authenticate by **redeeming a `link_token`** —
...
Status enum (`packages/esign/src/object-types/signature-request.ts`):
...
token, and emails ONE branded invitation via the `DeliverySink` (email channel).
...
`"document content hash mismatch"`, `"seal invalid: …"` — including
`"seal invalid: sealed document not found"` for a **legacy signature with no
PAdES-sealed blob** (a signature predating the sealing step); that reports
cleanly as an invalid reason, not an error). An **unknown
...
Status enum (`packages/esign/src/object-types/signature-envelope.ts`):
...
- `packages/esign/src/actions/signature-request/` — `create` / `send` / `view` / `sign` / `decline` inputs + guards.
- `packages/esign/src/actions/signature/` — `verify` / `certificate`.
- `packages/esign/src/actions/signature-envelope/` — `create` / `send` / `countersign` / `download` / `my-pending-countersigns` / `decline` / `void`, and `complete.ts` (the automatic completion step).
- `packages/esign/src/actions/member-signature/` — `set` / `get`.
- `packages/esign/src/object-types/signature-request.ts` / `signature-envelope.ts` — the status enums + `parts_frozen_when`.
- `packages/canon/src/types.ts` — `RequestContext` (the server-derived evidence seam).
- the platform's signing-protocol operations doc — the browser-portal surface: the external `/_sign` page, magic-link + PIN session, and the `/_canon/sign-token` + `/_canon/download-token` proxy endpoints.
