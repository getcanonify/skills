...
| `on_delete` | no | referential policy on target-side ownership links only; defaults to `restrict` when omitted |
| `fk_on` | no | which table holds the `via` FK column: `target` (default — parent→child) or `source` (a belongs-to / child→parent FK) |
| `part_of` | no | `true` marks the target as a constituent **part** of this root's identity (a compound object — docs/adr/0002) |
...
`on_delete` is enforceable only on a target-side ownership link
(`fk_on: "target"` or omitted). A source-side belongs-to link
(`fk_on: "source"`) is navigational: its FK lives on the source row, so
`typedDelete` does not walk it when deleting a parent. Declaring `on_delete` on
such a link is rejected by `catalog.refine_object_type` and `catalog validate`
with `on_delete_shape_invalid`; put the policy on the reverse target-side link.
...
The validator (`validateRefinement`) checks three things per link:

1. A source-side belongs-to link does not declare `on_delete` — else
   `on_delete_shape_invalid` rejects the inert policy and names the link.
2. The `target` ObjectType is registered — else
   `"Link "tasks": target ObjectType "task" is not registered"`.
3. The `via` column exists on the relevant FK side — else
...
`LinkOnDelete` is `restrict | cascade | set_null`. On a target-side ownership
link it is **data semantics, not handler semantics** — honoured by *every* path
that deletes a row: auto-CRUD's `<obj>.delete`, a custom `actions.declare`-d
delete step, and a manual `objects.delete`. You declare it once on the
target-side link. Source-side belongs-to links must omit `on_delete`; the
declaration is rejected because no deletion dependency can be enforced there.
...
## `part_of` — constituent parts of a compound (docs/adr/0002)
...
`part_of` carries these constraints, validated at `catalog.apply` (the diagnostic
...
| no owning a platform type | `part_of` **or any `on_delete`** on a link whose target is an ObjectType the **platform** registered (`org_member`, `_audit_event`, `signature_request`, …). Both hand the org control of platform rows: `part_of` makes them constituent parts, and `on_delete` is enforced on the target's table with or without `part_of` (`cascade` deletes those rows, `set_null` writes them). A link with neither still navigates — and the default `restrict` refuses YOUR delete while platform rows reference the row, rather than writing them | `platform_type_verbs_refused` |
