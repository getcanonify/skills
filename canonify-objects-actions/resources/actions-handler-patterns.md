...
`delete` · `assert` · `transition` · `call` · `for_each`. Each step is
discriminated by its `op`. Every step may carry an optional `when` predicate
guard (except `assert`, where the predicate is the mandatory `when`).
`read_many` binds a row set and `for_each` iterates it with a body containing
the other nine step kinds; the body cannot nest another `for_each`.
`transition` names one row as a subject of the action's declared `transition`
marker (Recipe 11) — it is the only step that writes nothing itself. `call`
runs a declared **fragment** — a named block of steps declared once with
`fragments.declare` — inline with bound inputs (Recipe 12).
...
`all_of` is the conjunction, and it is **guard-only** — `when` and `assert`
only, never a `where` map, whose columns already AND. Its members are leaves or
`any_of` groups, so the guard grammar is conjunctive normal form exactly one
level deep; `all_of` inside `all_of` is rejected, and there is still no `not`.
Prefer it to chaining `assert` steps when the conditions belong to one
decision, and always over count arithmetic — spelling AND as
`{ "ref": "$open.count", "eq": { "op": "-", "left": 2, "right": "$locked.count" } }`
is a correctness hazard, because one miscounted operand makes the guard pass
silently.
...
```json
{
  ...
  "when": {
    "all_of": [
      { "ref": "$day.locked", "eq": false },
      { "any_of": [
        { "ref": "$order.status", "eq": "open" },
        { "ref": "$order.status", "eq": "draft" }
      ] }
    ]
  },
  "else": { "code": "VALIDATION", "message": "dagen er låst" }
}
```
...
When `limit` is omitted, `read_many` probes one row past the cap. Matching
more than 500 rows fails with a `VALIDATION` envelope whose structured
`error.details` includes
`{ "code": "row_cap_exceeded", "step": <index>, "object": "<type>",
"as": "<binding>", "cap": 500 }`. The error message names the same object,
step, and cap. No partial `for_each` body runs, and writes made earlier in the
action are rolled back; the set is never silently truncated. An explicit
`limit` may request any positive value up to 500 when a bounded subset is
intentional.
...
## Recipe 11 — set transition from a `for_each` body

A transition action normally moves the one row its id input names. To move
every row of a set in one governed call — lock the day's open orders, close a
project's tickets — keep the `transition` marker, drop the id input, and let
the body name the rows with `transition` steps. The step writes nothing: after
the handler returns, the runtime guards every named row against `from`,
writes `to` on each and fires the on_enter/on_exit hooks per row, inside the
parent transaction.

```json
{
  "name": "order.lock_day",
  "description": "Lock every open order for one location and delivery date.",
  "object_type": "order",
  "input": { "location_id": { "type": "string" }, "delivery_date": { "type": "string" } },
  "governance": {
  ...
  "idempotency": "none",
  "transition": { "state_machine": "order.status", "from": ["open"], "to": "locked" },
  "handler": {
    "steps": [
      { "op": "read_many", "object": "order",
        "where": { "location_id": "$input.location_id", "delivery_date": "$input.delivery_date" },
        "as": "todays" },
      { "op": "for_each", "in": "$todays", "as": "o", "steps": [
        { "op": "transition", "object": "order", "where": { "id": "$o.id" },
          "when": { "ref": "$o.status", "in": ["open"] } }
      ] }
    ],
    "returns": { "candidates": "$todays.count" }
  }
}
```
...
- `object` must be the action's own `object_type`; `where` must match one row
  (`NOT_FOUND` otherwise). Naming a row twice moves it once; naming none is a
  successful no-op.
- One named row outside `from` fails the WHOLE call with `INVALID_STATE`
  (details carry that row's id and state) and nothing is written — decide
  membership with `when`, as above, rather than relying on the guard.
- Hook keys keep counting across rows: `<audit_event_id>:hook:0`, `:hook:1`, …
- Such an action cannot back an `automatic` transition (`bad_schedule` at
  `state_machines.register`): a schedule hands over one row id, and this action
  takes none.

## Recipe 12 — `call` a fragment declared once

A block that several handlers copy verbatim (a price lookup, an orderability
guard) is declared ONCE as a fragment and called with bound inputs. A fragment
is straight-line: reads, writes and asserts, no `for_each`, no `transition`,
no `call` of its own. It runs inside the calling action's transaction as the
calling principal, sees only its own `$input` (bound from the caller's `with`),
`$now` and `$principal`, and hands back its `returns` under `$<as>.<field>`.
Change the fragment and every caller changes on its next run — the callers
are re-type-checked before the new declaration lands.

Declare it (`canon fragments declare --input @price_lookup.json`, or a
`fragments/price_lookup.json` record in the bundle — `catalog.apply` applies
`fragments/` before `actions/`):

```json
{
  "name": "price_lookup",
  "description": "The customer price if one is in effect on the date, else the list price.",
  "input": {
  ...
  "steps": [
    { "op": "read_optional", "object": "price", "as": "cust",
      "where": { "product_id": "$input.product_id", "customer_id": "$input.customer_id",
                 "valid_from": { "lte": "$input.delivery_date" },
                 "valid_to": { "any_of": [{ "absent": true }, { "gt": "$input.delivery_date" }] } } },
    { "op": "read_optional", "object": "price", "as": "list",
      "where": { "product_id": "$input.product_id", "customer_id": { "absent": true },
                 "valid_from": { "lte": "$input.delivery_date" },
                 "valid_to": { "any_of": [{ "absent": true }, { "gt": "$input.delivery_date" }] } } },
    { "op": "read_aggregate", "object": "price", "count": true, "as": "custcnt",
      "where": { "product_id": "$input.product_id", "customer_id": "$input.customer_id" } }
  ],
  "returns": {
    "cust_amount": "$cust.amount_minor",
    "list_amount": "$list.amount_minor",
    "list_id": "$list.id",
    "cust_count": "$custcnt.count"
  }
}
```

Call it from any handler — here two actions share it:

```json
{ "op": "call", "fragment": "price_lookup", "as": "price",
  "with": { "customer_id": "$line.customer_id", "product_id": "$line.product_id",
            "delivery_date": "$input.delivery_date" } }
```

```json
{ "op": "update", "object": "order_line", "where": { "id": "$line.id" },
  "set": { "unit_price_minor": "$price.list_amount", "price_id": "$price.list_id" },
  "when": { "ref": "$price.cust_count", "eq": 0 } }
```

Rules:

- `with` binds every input the fragment declares as required
  (`call_input_missing`); an undeclared key is `call_input_unknown`; values are
  the flat value grammar (a literal, a `$ref`, one arithmetic/date node) and are
  type-checked against the input's declared type.
- A nullable binding (`read_optional`, a `when`-guarded step) may only flow into
  an input the fragment declares `optional` or `nullable` — otherwise narrow it
  with a `when: { ref: "$row.id", present: true }` on the call
  (`nullable_binding_into_non_nullable`).
- A field the fragment returns from a `read_optional` row is nullable at the
  call site too (`$price.list_id` above) — writing it to a non-nullable column
  is refused, exactly as a local `read_optional` binding would be.
- A `when`-skipped call binds `$<as>` to null, like any guarded step. Two
  conditions compose the same way they do elsewhere: guard a `read_optional`
  with the first, then guard the call on that binding's presence.
- `call` is legal at the top level and inside a `for_each` body (once per row).
- A fragment may not write a StateMachine-bound property (`direct_state_write`
  at `fragments.declare`, and again if such a machine registers later): it has
  no transition marker to route the write through.
- Re-declaring a fragment needs `expectedHash` (from `catalog.export`) or
  `force`; a change that breaks a caller (a field it reads is gone) is refused
  with `fragment_caller_broken`, naming the action. In a bundle, change the
  fragment and its callers together — apply re-declares fragments first and
  type-checks the new callers against the new fragment.
- Managed fragments prune like every other managed artifact; apply refuses,
  before any write, to prune a fragment a surviving action still calls
  (`unknown_fragment` for a bundle action, `fragment_in_use` for a tenant one).
...
- **A `transition` step without a `transition` marker, or naming an ObjectType
  other than the action's own** → `VALIDATION/invalid_transition_step`.
...
- **A `call` of an undeclared fragment** → `unknown_fragment`; a `with` that
  misses a required input → `call_input_missing`; an undeclared key →
  `call_input_unknown`; `$<as>.<field>` for a field the fragment does not
  return → `call_returns_no_field`. A fragment body itself rejects `for_each`,
  `transition` and `call` at decode.
...
- `predicate_any_of_flat` — `predicate any_of members cannot contain any_of or all_of`.
- `predicate_all_of_flat` — `predicate all_of members cannot contain all_of`.
- `value_expr_too_deep` — `value_expr_too_deep: a value expression may nest at most 5 levels`.
