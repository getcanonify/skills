...
catalog and falls back to English. **Generated-from-catalog** field labels come
from the ObjectType property's `label` and fall back to English humanization;
**enum values** come from the property's `valueLabels` and fall back to the
English humanized raw value with neutral color. The platform must not infer
tenant vocabulary by translating identifiers. See the [shared string ownership
contract](../../../DESIGN-SURFACE.md#24-user-visible-string-ownership).
