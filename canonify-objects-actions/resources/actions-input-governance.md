...
purely additive — an action that omits it keeps the renderer's default
heuristic (humanised name, no grouping, no confirmation).
...
- `per-key` — the caller's `idempotency_key` is the cache key; a retry with the
  same key replays the first envelope verbatim **for 7 days**. Good for
  destructive ops the caller retries on network failure.

  > ⚠️ **The replay window is 7 days.** Cached envelopes are pruned after that,
  > so a retry with the same key more than 7 days later **re-executes the
  > action** rather than replaying. For a destructive op that means it runs
  > twice. The window comfortably covers network-failure retries (the platform's
  > own longest retry chain is ~15 hours); it is not a long-term dedup ledger.
