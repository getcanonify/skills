...
| `object_type` | yes | the ObjectType this action is filed under — **one of your own**, never a platform type (see below) |
...
**A declaration may read a platform type, but never act on one.** A platform
ObjectType — `org_member` (over the users table), `signature_request` and the
other e-signature types, `_audit_event`, and anything backed by a `_`-prefixed
table — belongs to the platform: its verbs, its writes and its lifecycle are the
platform's. So `actions.declare` refuses (`VALIDATION`,
`details.code = platform_object_type_refused`) an action whose `object_type`,
`target.object_type` or `subject.object_type` is a platform type, or whose
handler *writes* one (an `insert` / `update` / `delete` / `transition` step),
and `state_machines.register` refuses a machine that governs one. Your handler
may still **read** a platform type (a `read` / `read_optional` / `read_many` /
`read_aggregate` step over `org_member`, say) — file the action under one of
your own ObjectTypes and read across. That read is decided when the action
runs, for whoever it runs as: it runs only if that principal could read the
platform type itself with no row filter, and is refused otherwise (`FORBIDDEN`,
`details.reason = PLATFORM_READ_NOT_AUTHORIZED`) — including whenever the action
runs unattended as the system principal (a schedule's target, a state-machine
hook or timer, an `on_fail` compensation). A derived property's expression may
not read platform data at all. Likewise, `schema.propose_migration` runs your
DDL but may not create, read or write a `_`-prefixed table, nor read a platform
type's view.
...
resolved by its manifest/form policy). That exemption covers the actions your
org declares and the verbs generated over your org's own types — not the
platform's actions (`catalog.*`, `outbox.*`, `org.*`, …) nor any verb acting on a
platform type (`org_member`, `signature_request`, `_audit_event`, …): those admit
`app`/`external`/`public` only where the platform opens them, so no access
manifest or form hands them to an app's users or a form's visitors. The newest of these, **`system`**, is the
...
transaction opened at all (`packages/canon/src/runtime-invoke.ts`, "SINGLE-STATEMENT
...
- **`email.send_template`.** The token is forwarded to the `DeliverySink` email channel as
...
The row is the one an id-shaped input field names (`<type>_id` or `id`).
An action that moves a SET instead carries no id input and names its rows
with `transition` steps in the handler (typically inside a `for_each`); the
runtime then guards, writes and fires hooks for every named row after the
handler, atomically. See
[actions-handler-patterns.md § Recipe 11](actions-handler-patterns.md#recipe-11--set-transition-from-a-for_each-body).
