...
Each entry: what it looks like, what it costs, the relevant authoring signal,
and the way out. Every one of these was found in a real, running, valid Canon.
...
**Authoring signal.** The declaration and link checks need an explicit upward
link; a record-page child table also needs that link or its own filter.
...
**Authoring signal.** Catalog discovery should find the platform primitive before
three or more columns on one table recreate its shape.
