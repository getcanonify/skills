...
description: The four obligations every Canonify domain model must meet — reuse the canonical object instead of retyping it, anchor every child to its owner, reuse the platform primitive before building one, and model time and money as effective-dated append-only rows with snapshotted settlements. States each as a rule with a worked before/after, and names the retained coherence-audit codes (C1-shadow-entity, C10-float-money) plus their authoring fixes. Read after the base `canonify` skill and before you declare your first table.
version: 1.0.0
---
...
one explains the authoring fix, and names a coherence-audit code only where the
retained audit still reports that defect.

| # | Obligation | Audit signal |
|---|---|---|
| 1 | **Reuse the canonical object.** Never re-declare an entity that already has an ObjectType. | `C1-shadow-entity` |
| 2 | **Anchor every child.** An FK column without an upward link is a defect. | Declare and validate the upward link |
| 3 | **Reuse the platform primitive.** Check before you model a capability. | Catalog discovery |
...
The declaration and link checks catch the missing anchor before a child surface
can rely on it. The authoring rule remains important even though the retired
generic advisory channel no longer reports this shape separately.

> A child table on a record page still needs a declared link or an explicit
> filter. The renderer cannot infer ownership safely from a bare table, so fix
> the model before adding the surface.
...
The platform catalog is the authority for these capabilities. Check it before
adding a hand-rolled implementation, then keep one link to the platform
primitive and derive the display values you need.
...
> **Declare it, don't hand-write it.** The platform owns this pattern:
> `catalog.refine_object_type` with `temporal: { from: valid_from, to:
> valid_to, key: [product_id] }` generates `<type>.set` (close the open period
> at the new `from`, insert, refuse any overlap — including inside an
> already-closed period) and `<type>.end`, creates the one-open-period-per-key
> index, and gives every read `as_of` plus the `$as_of` filter token. With
> `temporal`, the open end is `NULL` (the platform compiles the nullable
> comparison for you), so the sentinel below is only for rows you keep
> hand-modelled. Hand-write the verb only when it needs domain guards the
> generated one lacks — a hand-declared `price.set` replaces the generated one
> while everything else (`as_of`, `$as_of`, the index) still comes from the
> declaration. The rest of this section is what the declaration encodes.
...
- [ ] Does every `*_id` column have a declared upward link with `fk_on: source`?
- [ ] Did I run `canon objects list-types` and `canon actions list --json` before modelling this capability?
...
- [`canonify-app-lifecycle`](../canonify-app-lifecycle/SKILL.md) — the same
  ...
  mean, and the retained coherence checks. Obligation 4 is what makes its
  closed-period replay possible at all.
