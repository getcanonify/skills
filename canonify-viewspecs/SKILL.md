...
| `detail` | One record's properties as a labelled grid, with an optional master-detail record surface | `ObjectTypeSource` | `source`, `layout?`, `properties?`, `header?`, `links?`, `title?` |
...
empty array is a never-match), or a not-equal `{ "neq": ... }` (SQL `<>`).
...
(alias `"$actor"`) resolves at render time to the calling principal's id
...
and rejected at decode if it isn't in that closed set. Every filter key is a
...
**no** property (it counts rows); the other four require either a numeric
...
`unsupported_sortable_column_ref`. `emphasizeWhen` highlights cells when
predicates hold — same grammar as `showWhen`; each predicate field must name a
real property on the source ObjectType. The same declare-time check applies to
`emphasize[].when` and to column refs wherever a block exposes `columns`. Full
column grammar in
[resources/viewspec-binding-and-sources.md](resources/viewspec-binding-and-sources.md).
...
`layout: "flat"` opt-out when a single flat detail is intentional. An explicit
