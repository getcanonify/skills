...
A real screen can be several blocks composed together. You do that with the
`composite` block, which holds a list of `children` (each child is itself a
full ViewSpec). Composites nest, so you can build dashboards and compound
pages in one document. For an ordinary list-to-record surface, use one
standalone `detail`; its master-detail rail and record panel are the default.
...
## 1.1 Localization: who owns the words on the screen?

Use this ownership contract when a ViewSpec causes text to appear. The active
locale is resolved by user preference → org default → platform default. An
`@t:<key>` value is resolved from the org's locale bundle; it is not a literal
to show to the reader. A key must exist in every locale declared by the app;
use `@@t:` when the literal sigil itself is intended.

| Category | Owner | No declaration / fallback |
|---|---|---|
| **Fixed platform chrome** — counts, `Search`, `Filter`, empty-state frames, `Optional fields`, `Select a record`, and “not set” | The platform's typed locale catalogs (`chrome-en` / `chrome-da`), through `t()` / `tCount()`. | English `chrome-en` is the platform fallback. Do not add a tenant's enum or ObjectType name to this catalog. A null aggregate group uses this chrome key; give it an explicit enum value if it is business data. |
| **Generated-from-catalog text** — an empty state naming a type or a generated form-field label | Split ownership: the platform owns the sentence/frame; the canon author owns the inserted noun/field label via `display.entityKind` / `entityKindPlural` and `MappedProperty.label`. | The platform's acronym-aware English humanizer derives a compatibility label from the identifier. It never translates arbitrary ids through `chrome-da`; missing author labels are migration debt for a localized Canon. |
| **Enum values** — status/stage/channel chips, board columns, filters, and grouped dimensions | The canon author declares `valueLabels` beside `valueColors`; each value can be a literal or an `@t:` key. A StateMachine `StateMeta.label` is the same author-owned source for its states. | `humanizeEnumLabel(raw)` is the English compatibility fallback, with neutral color. Stored values and filter/sort semantics stay raw. The platform locale catalog does not guess tenant enum vocabulary. |
| **Action labels** — `button`, `panel` submit, row/menu actions, and generated CRUD captions | The canon author owns the final caption through ActionType `presentation.label`, `display.actions`, or the ViewSpec `label` / `submitLabel`. | An unlabeled generated CRUD action may compose a platform-localized verb with the author's localized ObjectType name. If neither is declared, acronym-aware English humanization of the action id is the legacy fallback. |

An explicit ViewSpec label wins for that view; otherwise the ActionType/ObjectType
declaration supplies the shared caption. The platform may supply grammar and a
compatibility default, but it must not silently turn a database noun into a
supposed Danish translation. The declaration is the default, so a view need not
restate it: a column with no `label` reads the property's `label`, a `panel`
with no `title` / `submitLabel` reads the action's `presentation.label`, and a
`detail` with no `properties` reads `display.primaryProperties`.

For migration, add localized ObjectType names, property `label`s, every visible
enum `valueLabels` entry, and labels for surfaced custom actions. Keep the raw
enum values and action ids stable. Existing English defaults are retained only
as compatibility fallbacks while a Canon is brought up to this contract; do not
translate generated strings into the Pax bundle.

The detailed product contract, including an author → `/v1/schema_describe` →
`useSchemaCatalog` → `AutoList:Cell` enum trace, is in the platform
...
|---|---|---|---|
| `table` | A list — one row per record, or one row per group (an aggregate source, which may also carry `compare` + `window`) | `ObjectTypeSource` *or* `AggregateSource` | `source`, `columns` (≥1), `rowActions?`, `presets?`, `page_size?`, `compare?`, `window?`, `title?` |
| `detail` | One record's properties as a labelled grid, with an optional master-detail record surface | `ObjectTypeSource` | `source`, `recordLayout?`, `properties?`, `header?`, `links?`, `title?` |
...
| `metric` | A single KPI tile (e.g. "Revenue: 42k") — with a time-bucketed `groupBy` spine it gains a sparkline (`window`) and a delta badge (`compare`), all from one read | `AggregateSource` (scalar, or a time-bucketed `groupBy`) *or* a record-scoped `{ link, aggregate }` | `source`, `label`, `format?`, `compare?`, `window?`, `title?` |
| `chart` | A bar or line chart of one measure across one dimension | `AggregateSource` (**with** `groupBy`) | `source`, `kind?`, `format?`, `stacked?`, `compare?`, `partialTrailing?`, `window?`, `title?` |
...
- **`title` — the block's name, painted once.** Every variant accepts an
  optional `title` string, painted in the block's own header (a `composite`
  paints it as a section header above its children, and a child whose title
  repeats its section's or its tab's name paints no title of its own). On
  `button`, `label` is the button control caption (text on the button
  itself); `title` is the separate caption above it. On `metric`, `label` is
  the required tile caption; `title` is the optional tile header.
- **`composite` layout axis.** `composite` accepts an optional `layout`:
  `"stack"` (default, vertical), `"grid"` (responsive columns, with optional
  `{ span: 2 | 4, view: <ViewSpec> }` wrappers for width), or `"tabs"` (each
  ...
  filter row (`{ property, kind: "date-range" | "dimension" | "reference" | "date", required? }`) that scopes
...
`gt` / `gte` / `lt` / `lte`, or a set-membership `{ "in": [...] }` (SQL `IN`;
an empty array is a never-match).
Every literal and operator operand is checked against the property's declared
type at declaration time: booleans use `true`/`false` (legacy numeric `0`/`1`
are also accepted and normalized for storage), numbers use numbers, enums use
declared values, and timestamps use ISO strings. A value of `"$principal"`
resolves at render time to the calling principal's id
(auto-scope a "my records" view); an unresolved token is dropped. Range bounds
may be a literal or a **relative-date token** (`$now-30d`).
...
of `:this_week`, `:this_month` (e.g. `{ "created_at": ":this_week" }`),
resolved server-side to a date window and rejected at decode if it isn't in that
closed set. Every filter key is a
closed path — a direct property, one or two to-one link hops, and a typed
terminal; a to-many path is rejected with `link_path_to_many` and an unknown
path with `unknown_column_ref`. `sort` names a direct property (`"name"` for
ascending; `"-name"` for descending) and is also validated at declare time.
...
The four operations are `count`, `sum`, `avg`, `max`. `count` takes
**no** property (it counts rows); the other three require either a numeric
`property` or, for `sum`, a 1–3-factor numeric `of` product. Add `groupBy` to
get one row per group (a grouped table); leave it off to get one scalar (a
metric). Its `filter` uses the same closed path grammar as `groupBy`: a direct
property, one or two to-one link hops, and a typed terminal. A to-many filter
path is rejected with `link_path_to_many`; unknown paths fail with
`unknown_column_ref`. Use `expand` for the one declared to-many fan-out and
at most one correlated `lookup` factor. Every traversed ObjectType is
policy-gated like a direct read. Full path, measure, and lookup rules live in
[binding and sources](resources/viewspec-binding-and-sources.md) and
[analytics](resources/viewspec-analytics.md).
...
time for property-type compatibility). `sortable: true` is only legal on a
**bare property ref** — a dot-traversal (`"organization.tier"`) is a display
projection, not a sortable source property. Relation-path sorting is not
supported by design because list sorting does not perform relation joins or
per-row reads; denormalize the value onto the source ObjectType or sort by a
local property instead. Such a column is rejected at declare time with
`unsupported_sortable_column_ref`. `emphasize` applies semantic status rules
when predicates hold; each predicate field must name a real property on the
source ObjectType. The same declare-time check applies to `emphasize[].when`
and to column refs wherever a block exposes `columns`. Full
column grammar in
[resources/viewspec-binding-and-sources.md](resources/viewspec-binding-and-sources.md).
...
same grammar to pre-fill an Action input from the row in scope. In a table
`rowActions` entry, `$row.<field>` is a property binding and the field is
checked against the table's ObjectType at declare time; `$context.*` values and
literals remain render-context/input values. Row-action `showWhen` predicates
are evaluated against the row, so their fields are checked against that same
ObjectType too.
...
**Master-detail** — a standalone `detail` is the default list → record surface;
its `source` derives the rail and selected record panel. Use the explicit
`recordLayout: "flat"` opt-out when a single flat detail is intentional. An explicit
`composite` with a `table` and `detail` remains an escape hatch for bespoke rail
columns or row actions, not the normal recipe. See
...
`showWhen`: an array of equality predicates (`{ field, op: "eq", value? }`). The
...
it — but a re-declare of an EXISTING name is guarded by optimistic
concurrency (`expectedHash?`/`force?` on the input; a bare re-declare of an
existing View is refused before any write, `CONFLICT/HASH_REQUIRED`; see
[resources/viewspec-forms-and-actions.md](resources/viewspec-forms-and-actions.md#applying-a-view-view_specsdeclare)).
This is **isomorphic**: the exact same call works on all three surfaces.
...
**Every probe runs under the CALLER's own read authority**, exactly as a live
render would: `source.row_count` / `source.value` / `source.group_count` are
computed with the caller's row filter applied, and a `source.filter` naming a
column the caller's policy masks or denies is rejected the same way an
unknown column is — it never leaks a count computed over rows or columns the
caller could not otherwise see. Preview a spec as the principal who will
actually view it if that principal is row- or column-restricted; previewing
as an unrestricted principal (e.g. owner) can show a higher `row_count` than
a restricted viewer will ever see.
