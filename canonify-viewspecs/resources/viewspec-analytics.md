...
- **Empty buckets** are decided at the read, so no view has to guess: a bucket
  ...
  **omitted** for `avg`/`max` (no observations is not zero — it's nothing
  to average).
...
- In a **ViewSpec**, declare `window: { last: N }` on a `metric`, a `chart` or a
  grouped `table` — the same field on all three (see *`window`* below). ViewSpec
  authors rarely reach for the raw form.
...
- **It pads the ends.** For `count`/`sum`, an empty leading or trailing bucket
  *inside the requested window* comes back as a true `0`, so **exactly N points**
  ...
  intended range. `avg`/`max` are **not** padded — a period with no rows
...
layers are derived from a widened, zero-filled scan, and `avg`/`max`
...
ranges, `{ in }`, and the relative-date tokens (see
[viewspec-binding-and-sources.md](viewspec-binding-and-sources.md), *Filters*) —
including the same declared-property type checks and boolean `true`/`false` →
SQLite `1`/`0` storage normalization. The render honors all of it:
`{ "score": { "gte": 9 } }` on a count metric
...
Every analytics filter key uses the closed path grammar from the binding
reference: `property`, `link.property`, or `link.link.property`. Each
non-terminal segment must be a declared to-one link, and the terminal property's
type supplies the literal check. A to-many link is never legal in a filter key;
declaration fails with `link_path_to_many`. Unknown segments fail with
`unknown_column_ref`. This is the same rule used by record sources and table
filter presets.

Every traversed ObjectType is policy-gated exactly like a direct read of it.
The read applies each join target's row filter and column projection at the
join alias; lookup rows receive the same treatment. A principal who cannot
read a traversed type gets a loud read failure instead of a result that could
be used to bypass policy.
...
### Link paths, `expand`, and fan-out

The raw `objects.aggregate` read and ViewSpec authoring accept a closed, dotted
property path in `group_by`, `groupBy`, and filter keys. There are at most two
link hops before the terminal property. A to-many path is rejected unless it is
under the one explicit `expand` prefix.

`expand` names the one path whose final hop is to-many; it is the only place
fan-out is allowed. Earlier hops must be to-one. To-one hops use LEFT JOINs;
the final expanded hop uses an INNER JOIN, so a missing component contributes
no expanded row while a nullable ordinary link can still produce a NULL group.
There can be only one expanded many-hop, and an `expand` that no group, factor,
or match path uses is rejected with `expand_unused`.

Once a source expands, use `aggregate: { "op": "count" }` or a labeled
`aggregate.of` measure; a plain `sum` is rejected with `expand_requires_measure`.
The rule exists because summing a root property after a fan-out would multiply
it by the number of related rows. The expanded source is still one aggregate
read, not a client-side per-row loop.

`@bucket` remains authoring shorthand and is parsed at the edge into
`group_by` plus `group_by_bucket`; it is not part of a wire path.

| Surface | Linked aggregate form |
|---|---|
| CLI | `canon objects aggregate order_line --op count --group-by product.name --where '{"product.name":"Lunch box"}' --expand product.components` |
| REST | `GET /v1/objects/order_line/aggregate?op=count&group_by=product.name&where=%7B%22product.name%22%3A%22Lunch%20box%22%7D&expand=product.components` |
| MCP | `objects.aggregate({ object_type: "order_line", op: "count", group_by: "product.name", filter: { "product.name": "Lunch box" }, expand: "product.components" })` |

### Derived measures: `aggregate.of` and `lookup`

`aggregate.of` is a 1–3-factor measure; each factor is numeric and at most one
factor may be a `lookup`. It is a product summed over the source rows, so the
factor paths can follow the same to-one prefixes and the one declared `expand`
path as `groupBy` and filters. `of` is valid only with `op: "sum"`; do not send
both `property` and `of`. A table or chart must give the product a non-empty
`aggregate.label`, because there is no single source property to use as its
measure heading.

A lookup factor is an explicit correlated read:
...
Lookup `match` is equality-only. The match keys and values are checked against
the lookup and source terminal property types, the selected lookup property
must be numeric, and `default` must be finite. Multiple matching lookup rows
resolve deterministically to the maximum selected value. Missing matches use
the declared default. The lookup ObjectType is policy-gated exactly like every
join target, so a lookup never becomes a policy bypass.

### Worked example: the PortionPlan

This is the complete derived-read shape: quantity × component default ratio ×
customer override, grouped by the expanded component slot and scoped to the
record context. The `$context.*` values are resolved by the aggregate read;
they are not interpolated into SQL by the renderer.
...
{ "type": "table",
  ...
  "source": {
    ...
    "groupBy": "product.components.slot",
    "aggregate": {
      "op": "sum", "label": "Portions",
      "of": [
        "quantity",
        "product.components.default_ratio",
        { "lookup": { "objectType": "customer_component_ratio",
                      "match": { "customer_id": "order.customer_id",
                                 "component_id": "product.components.component_id" },
                      "property": "ratio", "default": 1 } } ] }
  },
  "columns": ["product.components.slot"] }
```

Fixture math: at location `L1` on `2026-09-07`, P1 has C1 (`salad`, `0.5`)
and C2 (`hot`, `1.0`); P2 has C2 (`hot`, `0.8`). Customer A has a C1
override of `2.0`. Locked lines are A/P1 quantity 10, B/P1 quantity 4, and
B/P2 quantity 5. An open P1 quantity 100 and a locked order at L2 are
excluded by the three path filters. Therefore PortionPlan fixture math:
salad = 10×0.5×2.0 + 4×0.5×1 = **12** and hot =
10×1.0×1 + 4×1.0×1 + 5×0.8×1 = **18**. This exercises the fan-out, one
lookup hit, the lookup default, and all three link-path filter keys.
...
### The stat tile — a spine, a window, a compare

A metric carries the SAME aggregate envelope a `chart` and a grouped `table`
carry: `source` (optionally with a time-bucketed `groupBy` spine), `window` and
`compare`. With a spine the tile reads ONE bucketed aggregate and derives
everything from it:

- the **figure** is the newest bucket (the bucket containing now, when the
  measure zero-fills; the newest observed bucket otherwise);
- the **sparkline** is the window — `window: { last: N }` draws the newest N
  buckets in grey with the current period marked in the accent (see *An
  explicit window* above for what the read pads);
- the **delta badge** is that newest bucket against its own comparison layer:
  `compare.offset` gives "+12%" vs the same bucket a period ago, `compare.rolling`
  vs the trailing mean (when both are declared the offset wins). The badge keeps
  its sign; a `null` or zero reference renders an em-dash. Beside it the tile
  names the layer in the chart legend's own words ("Same month last year").
- a newest bucket the read marks `partial` is admitted in words beside the
  figure — the same caveat the chart carries as texture.

```json
{
  "type": "metric",
  "source": { "objectType": "order", "aggregate": { "op": "sum", "property": "total" }, "groupBy": "created_at@month" },
  "label": "Revenue",
  "format": "currency",
  "window": { "last": 12 },
  "compare": { "offset": { "period": "year", "periods": 1 } }
}
```

That is the full stat-tile shape: **value + delta + sparkline**, from one
request. The envelope's rules are the chart's, with the same codes
(*`compare`* and *`window`* below): a spine with a `@bucket`, an additive op for
`compare`, no `seriesBy`, no `source.filter` on the spine property.

**A metric renders one figure.** A categorical `groupBy` (`"status"`) or a
`seriesBy` is rejected (`invalid_metric_group_by`) — a split has no "current
period" to headline. When you want axes, a tooltip and every bucket readable,
that is a `chart` block over the same source, below.
...
| `compare` | no | `{ offset?, rolling? }` — a prior-period layer and/or a trailing rolling mean. Requires a bucketed `groupBy` and an additive op. Shared with `metric` and the grouped `table`. See *Compare* below. |
...
| `window` | no | `{ last: N }` — draw only the newest `N` buckets. Requires a bucketed `groupBy`. Shared with `metric` and the grouped `table`. See *Window* below. |
| `title` | no | The chart's block-header name (default: `<Type> by <Dimension>`). |
...
- **High cardinality folds.** Beyond 12 categories the tail folds into a single
  **Other** mark — summed for `count`/`sum`, max-composed for `max`.
...
| Rejected | Why |
|---|---|
| `stacked` with `avg` / `max` | Only `count`/`sum` are additive over a partition. The sum of per-series averages is not an average of anything. |
...
- **Past 5 series the tail folds into Other**, using the same op-correct
  combiner: summed for `count`/`sum`, max-composed for `max`, and an
...
series, over a **time-bucketed** `groupBy`. It is part of the **aggregate
envelope** — the `source` + `compare` + `window` triple that `metric`, `chart`
and a grouped `table` all carry, with one rule set and one set of codes — and
the raw aggregate read speaks the identical concept as the `compare_period` /
`compare_periods` / `rolling` params documented above. On a chart the layers
draw as extra strokes; on a metric the newest bucket's layer becomes the delta
badge; on a grouped table they become Prior / Rolling columns.
...
Declare rejects a `compare` that cannot render, all under `invalid_aggregate_compare`
except the offset/spine mismatch, which gets its own code:

| Rejected | Code | Why |
|---|---|---|
| `groupBy` has no `@bucket` tail | `invalid_aggregate_compare` | Compare layers are derived by CALENDAR arithmetic over buckets — meaningless on a categorical spine ("3 statuses ago"). |
| `compare` together with `seriesBy` | `invalid_aggregate_compare` | Compare is the SAME measure at another time; `seriesBy` is a categorical split into different entities. v1 does not compose the two. |
| `source.aggregate.op` is `avg`/`max` | `invalid_aggregate_compare` | The layers are derived from a WIDENED, zero-filled scan; only `count`/`sum` are additive over a partition. |
| `source.filter` constrains the spine's own bucketed property | `invalid_aggregate_compare` | That filter would cut the widened scan the layers are derived from — `offset`'s shifted buckets AND `rolling`'s warm-up — so the cut buckets would zero-fill and show as real data. Time-scope a compared chart with `window`, not `source.filter`. The raw aggregate read rejects the same combination (`compare` + a filter on the `group_by` property) with a `VALIDATION` 400. |
| `compare.offset.period` outside the admitted set for the spine's bucket | `invalid_aggregate_compare_offset` | The `(bucket, period)` compatibility table below. |
...
The author-facing twin of the raw read's `window_last` param (*An explicit
window*, above), and the second half of the aggregate envelope: `window:
{ last: N }` draws only the newest `N` buckets, zero-filled to the window edges
for `count`/`sum`, instead of every bucket the data spans. The same field on a
`metric` sets its sparkline's length, and on a grouped `table` its rows.
...
categorical spine with `invalid_aggregate_window` (the read rejects the identical
...
the chart doesn't draw. A standalone grouped `table` declares the same two
fields directly.
...
- **`partial: true` is a read-layer fact, not a chart-only one.** It is
  ...
  metric sparkline, or the chart's table twin — whether or not `compare` was
