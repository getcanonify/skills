...
- `aggregate.op` — one of `count`, `sum`, `avg`, `max`.
- `aggregate.property` — **required** for sum/avg/max; **forbidden** for
...
- `filter` (optional) — the same equality, range, and set-membership grammar as
  `ObjectTypeSource.filter`; literals are checked against
...
fields in `emphasize[].when` must be direct properties on the
...
| `{ "ref": "status", "emphasize": [{ "when": [{ "field": "status", "op": "eq", "value": "overdue" }], "level": "danger" }] }` | Conditional semantic emphasis |
| `{ "ref": "status", "emphasize": [{ "when": [...], "level": "danger" }] }` | Semantic status emphasis — ordered `good`/`warn`/`danger` rules, first-match wins |

Link aggregates: `count` takes no `:property`; `sum` / `avg` / `max`
...
`emphasize` flags a cell's **semantic state** on a warm status ramp. It's an
...
source ObjectType. The semantic level is the only cell-emphasis treatment.
Rendered by `AutoList` only.

**Known gap.** `when` reuses the FormSpec Predicate grammar, whose ops today are
`eq` only — there is **no** `gt`/`lt`/`gte`/`lte`. So a
...
## Filters: scalars, ranges, and sets

Each `filter` entry is one of three shapes: an equality scalar, a range
predicate, or a set-membership (`{ in }`) predicate.
...
- Relative-date token: `$now`, `$now-30d`, `$now+1w` — usable as an equality
  value, a range bound, or an `{ in }` member. Units are
...
- Relative-date **equality token** (curated): in an **equality** position a
  `:`-prefixed value must be one of a closed set — `:this_week`, `:this_month` — e.g.
  `{ "created_at": ":this_week" }`. Each resolves server-side to a calendar
  window (`[start, end)`, UTC, weeks start Monday). An unknown `:token` is rejected **at decode**. Distinct from
  the `$now±<duration>` range tokens above: the `:token` set is fixed and valid
  only as an **equality** value; a `:token` in a range/`{in}` position
  is not accepted.

Each op object is shape-pinned: a stray key alongside `in` or a range
...
### `$principal` — the calling principal's id

A filter value of `"$principal"` resolves at render time to
