...
## Labels, locale, and fallback ownership

The panel and button are part of a localized screen, so their words follow the
same ownership contract as the rest of ViewSpec:

- **Fixed platform chrome** — `Search`, `Filter`, `Optional fields (N)`,
  `Select a record`, empty-state frames, and the submit/cancel scaffolding come
  from the platform's `chrome-en` / `chrome-da` catalogs. A missing key falls
  back to English `chrome-en`; it is not a canon label.
- **Generated-from-catalog text** — a generated auto-CRUD field label is owned
  by the canon's `MappedProperty.label`; the platform's form frame is its own
  locale-catalog key. With no property label, the platform's English
  humanize fallback is retained for compatibility and is migration debt for a
  localized Canon.
- **Enum values** in generated selects and chips are owned by the canon's
  `valueLabels` map, beside `valueColors`. Use a literal or `@t:<key>` per
  value. With no entry, the raw value is humanized in English; it is never
  looked up in the platform chrome catalog.
- **Action labels** are canon-owned: declare `presentation.label`, an
  ObjectType `display.actions` override, or the ViewSpec `button.label` /
  `submitLabel`. `@t:<key>` resolves in the active org locale. For an
  unlabeled generated CRUD action, the platform may localize the verb and use
  the author's localized ObjectType name; otherwise the legacy English action
  humanizer is the fallback.

An explicit ViewSpec label is local to that ViewSpec and wins over the action's
baseline presentation label. A literal is not translated automatically, and a
missing `@t:` key is a manifest/locale completeness problem rather than a
reason to display the raw key. Existing Canons should add these declarations
and locale-bundle entries without renaming stored values or action ids.
...
- `submitLabel` (optional) — overrides the default submit caption, which is
  the Action's `presentation.label` (else the platform's localized Submit). The
  panel's `title` defaults to the same caption.
...
Every block accepts `showWhen`: an array of equality predicates using the
grammar `{ field, op: "eq", value? }`.
...
```json
{
  ...
  "children": [
    ...
    },
    {
      ...
      "action": "deal.advance",
      "label": "Advance stage",
      "showWhen": [{ "field": "stage", "op": "eq", "value": "open" }]
    }
  ]
}
```
...
- `name` — a dot-separated handle (e.g. `Deal.detail`). Re-declaring the same
  name overrides the previous spec — but see *Optimistic concurrency* below:
  a re-declare of an EXISTING name is guarded, and a bare re-declare (no
  `expectedHash`/`force`) is refused before any write.
...
`unknown_column_ref`, `unknown_action`, `unsupported_sortable_column_ref`
(a link-traversal or link-aggregate column marked `sortable: true` — see
*Column references* in the binding-and-sources resource).

**Optimistic concurrency — `expectedHash` / `force`.** A re-declare of an
EXISTING `name` is guarded by optimistic concurrency on the View's content
hash, default-on: omit both `expectedHash` and `force` and the call is
rejected before any write (`CONFLICT/HASH_REQUIRED`, `details.current_hash`)
rather than silently last-writer-wins. A FRESH `name` has nothing to conflict
with, so the guard never fires on first declare. Pass the hash read from
`catalog.export`'s per-view `declaration_hash` as `expectedHash`; a mismatch
is `CONFLICT/STALE_DECLARATION` naming which top-level keys drifted (e.g.
`['spec']`). `force: true` skips the check as a deliberate overwrite;
supplying both is rejected as contradictory intent
(`VALIDATION/CONTRADICTORY_DECLARATION_INTENT`). Same contract as
