...
`recordLayout: "master-detail"` is shown explicitly for clarity; omit
`recordLayout` and
...
Use `recordLayout: "flat"` only when the record should remain a single flat detail:
...
This is a spatial opt-out, not a links contract opt-out: `recordLayout: "flat"`
...
band (title / subtitle / status chip) — see
...
record page — which looks like a scoping bug. Declare the missing many-link, or
add an explicit filter; the renderer cannot infer ownership safely.
...
but with **no** declared detail/composite anywhere still lands on the inferred
page. Declare a record view when the authored surface matters.
...
- **Header refs traverse links.** A declared `header` whose `title` / `subtitle` /
  `status` follows a link (`organization.name`, or a `{organization.cvr}`
