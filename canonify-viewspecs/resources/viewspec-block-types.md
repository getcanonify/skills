...
calendar token, range, `{in}`, `{neq}`). Boolean literals use `true`/`false`
...
and ANDed. So a table scoped `{ lifecycle_state: { neq: "churned" } }` CAN
carry a working `{ lifecycle_state: "churned" }` chip — the chip wins on that
key — while a chip on a different property narrows within the scope. (Read
path: `AutoList` seeds the `where` from `scopeFilter`, then overwrites per key
from the filter-bar state.) One authoring caveat: a `{neq}` preset has no
filter-bar chip kind and is silently skipped — don't declare one.
...
use the ObjectType's default grid.

`layout` is optional: omitted and `"master-detail"` use the record surface;
...
names the Action; `submitLabel` and `presetParams` are optional. The canon owns
an explicit `submitLabel`; fixed platform chrome around the form comes from the
active locale catalog. When no label is supplied, an auto-CRUD form uses the
ActionType/ObjectType display contract and retains English humanization only as
...
bounded horizontal region. Set `columnWidth` to a size token (`xs` through `xl`)
or a fixed CSS length such as `280px` to opt into fixed-width scrolling for
every column; fractions are intentionally not accepted for this override.
