...
An enum with no declared `valueColors` renders with a **neutral tint
everywhere** it appears. Automatic color is not inferred from the enum value.
If a delivery channel, loyalty tier, or other enum should retain semantic
colors, declare `valueColors` on the ObjectType; the declared mapping is the
source of that color across the ViewSpec blocks.

## Enum value labels and localization

`valueColors` controls appearance; `valueLabels` controls the displayed words.
The canon author owns the label map at the same ObjectType property declaration
site, and may use `@t:<key>` for a locale-specific value. The renderer never
uses the platform's fixed chrome catalog to guess a tenant enum. An undeclared
value falls back to the English humanized raw value for compatibility and keeps
the neutral tint; a localized Canon should declare every value that reaches a
user-facing block. The stored value remains the filter/sort key.
...
`columns` must have at least one entry. A column with no `label` reads the
property's declared `label` (a `<link>.<prop>` traversal column reads the FK
column's declared label, else the link target's `entityKind`), resolved in the
active locale — declare `label` only to override, never to restate the
property's own name.
...
reuses the same typed source-filter grammar (equality scalar, `:this_week`
calendar token, range, `{in}`). Boolean literals use `true`/`false`
and are normalized to storage values at read time; legacy numeric `0`/`1` are
also accepted. Every preset key uses the closed path grammar: a direct
property, `link.property`, or `link.link.property`, with a typed terminal and
at most two to-one hops. A to-many path is rejected with `link_path_to_many`;
unknown paths fail with `unknown_column_ref`. It is a DECLARED set, not
user-saved state. An empty `label` is rejected at decode.

**How a chip combines with `source.filter` — per-key REPLACE, cross-key AND.**
Tapping a chip writes its entries over the view's `source.filter` map by
property name: a chip touching a property the source filter also constrains
**replaces** that constraint; properties the chip does not name are preserved
and ANDed. A chip on a different property narrows within the scope. (Read
path: `AutoList` seeds the `where` from `scopeFilter`, then overwrites per key
from the filter-bar state.)
...
(the subject id auto-binds). Each `$row.<field>` field is checked against the
table's source ObjectType at declare time; `$context.*` values and literals are
resolved at render time. Because row-action `showWhen` is evaluated against the
row, its predicate fields are checked against the same source ObjectType.
`default: true` marks the action that stays inline
...
A labelled property grid for a single record. At the top level, `detail` also
owns the default master-detail record surface: the platform derives the list
rail from its `source` and shows the selected record in one panel. A record id
comes from the render context (`$context.<objecttype>.id`, falling back to
`$context.id`) once a record is selected. `properties` is optional — omit it to
render `display.primaryProperties` above the fold (the remaining properties fall
to "More details"). An entry with no `label` reads the property's declared
`label`; declare a label only to override it for this view.

`recordLayout` is optional: omitted and `"master-detail"` use the record surface;
`"flat"` opts out of that spatial shell. Flat does **not** disable the links
contract. `links[].as` still presents related records as `"tab"`, `"card"`, or
`"chip"`; the renderer-owned `Overview` panel contains the property grid and
cards/chips when linked presentations exist. See
[viewspec-master-detail.md](viewspec-master-detail.md) for the mobile list and
record states and the explicit composite escape hatch.

```json
{
  "type": "detail",
  "source": { "objectType": "customer" },
  "recordLayout": "master-detail",
  "properties": [
    "name",
    "email",
...
title/subtitle/status-chip band above the property grid, each a field ref:

```json
{
  "type": "detail",
  "source": { "objectType": "customer" },
  "header": { "title": "name", "subtitle": "email", "status": "tier" }
}
```
...
names the Action; `title`, `submitLabel` and `presetParams` are optional. Both
`title` and `submitLabel` default to the Action's `presentation.label`,
resolved in the active locale, so a panel never restates its action's own
caption — declare either only to override. An Action with no
`presentation.label` renders no heading and the platform's localized Submit;
fixed platform chrome around the form always comes from the active locale
catalog.
...
the default caption. `label` is canon-owned display text and may be an
`@t:<key>` reference. If it is omitted, the ActionType presentation label wins;
an unlabeled generated CRUD action falls back to a localized verb plus the
ObjectType display name, then to legacy English humanization.
...
is `{ "property", "kind": "date-range" | "dimension" | "reference" | "date", "required"? }`) that scopes every child
...
An `AggregateSource` — one number. `label` is required; `format` is an optional
render hint. The source may instead be a record-scoped one-hop aggregate
`{ link, aggregate }` (see [viewspec-analytics](viewspec-analytics.md), which
has a dashboard-lead example).
...
do. Give its source a time-bucketed `groupBy` spine and the tile reads ONE
bucketed aggregate: the **newest bucket** is the figure, `window: { last: N }`
draws the newest N buckets as a sparkline under it, and `compare` turns that
bucket's own comparison layer into a delta badge — `offset` (the same bucket a
period ago) or `rolling` (the trailing mean). A newest bucket the read marks
`partial` says so beside the figure.
...
The badge keeps its `+`/`-` sign; a `null` or zero reference renders an
em-dash rather than Infinity. The envelope's rules are the chart's
(`invalid_aggregate_compare`, `invalid_aggregate_window`,
`invalid_aggregate_compare_offset`); a metric additionally refuses a
categorical `groupBy` or a `seriesBy` (`invalid_metric_group_by`) — one tile,
one figure, one series.
...
### Three chart-only fields: `compare`, `partialTrailing`, `window`

A time-bucketed chart (`groupBy` carries an `@bucket` tail) can also declare:

- **`compare`** — draw a same-measure comparison layer alongside the base
  series: `offset` (a prior-period copy, e.g. "same month last year") and/or
  `rolling` (a trailing rolling mean). Requires an additive measure
  (`count`/`sum`) and excludes `seriesBy`.
- **`partialTrailing`** — opt OUT (`false`) of the automatic mark every
  bucketed chart gets on its trailing bucket when that bucket contains "now".
  Default is `true` (on) even when the field is absent.
- **`window: { last: N }`** — draw only the newest `N` buckets, zero-filled to
  the window edges, instead of every bucket that has data.
...
```json
{
  ...
  "compare": { "offset": { "period": "year", "periods": 1 } },
  "window": { "last": 12 },
  "title": "Revenue by month, vs. same month last year"
}
```

All three fields, their validation rules, and how the comparison layers render
are documented in full in
[viewspec-analytics.md](viewspec-analytics.md#compare--same-measure-comparison-layers).
...
per-card action triggers. `title` is optional and is painted in the board's
block header. `source` must be an `ObjectTypeSource` (not aggregate-backed).
...
Columns are responsive by default: they share the available width while each
stays at least `16rem`, which keeps a 14px two-line card title and compact card
metadata readable. If the minimum does not fit, the board scrolls inside its
...
`number` | `currency` | `percent`), and an optional `peek` naming a
...
```json
{
  ...
  "empty": { "title": "Queue clear", "body": "All caught up." }
}
```
...
`detail`, and an app that navs them — ships as the `queue-demo` example
bundle in the Canonify platform repo.
