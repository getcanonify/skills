...
`{ "span": 2 | 4, "view": <ViewSpec> }` to claim that many columns (a wide
table next to two narrow metrics). A plain child takes one column — there is
no `span: 1`, the default is the unwrapped child. `span` is only valid under
`"grid"`.
