...
| `confidential` | Set `true` for sensitive types; production policy still requires an explicit grant, denials remain `FORBIDDEN`/`denied`, and granted mutations are audited as `sensitive`. Omitted → ordinary audit treatment. |
| `idPrefix` | Prefix for generated ids. Omitted → the generated ObjectType's `rec_` fallback (or `Prefix.fromTable` in lower-level typed writes). Effectively write-once for existing rows. |
| `properties.<name>` | Narrow a `mapped` property's type (e.g., `string` → `enum`), or surface a column that the default registration hid. |
| `properties.<name>.description` | Field-level prose surfaced in `schema_describe` and discovery under `fieldDescriptions`; preserved through export/apply. |
...
| `rollups` | Declared root summary properties — each computes an aggregate over a `part_of` child link (compound objects, docs/adr/0002). |
...
| `temporal` | Effective dating — `{ from, to, key }` names the period bounds and the series key; generates `<type>.set` / `<type>.end`, enforces one open period per key in the schema, and enables `as_of` reads and the `$as_of` filter token. |
| `defaults` | Per-verb opt-out for auto-CRUD: `{ delete: false }` makes the row undeleteable on every surface; `{ replace: false }` suppresses a compound root's `<name>.replace`. On a type without `part_of` children, `replace:false` is a valid no-op because that action is not generated. |
| `display.searchable` | Ordered filter-bar properties. Omitted → string/enum properties among inferred `primaryProperties`. Display metadata, not policy. |
| `display.defaultFilter` | Renderer-owned default for browse/reference-picker lists. Omitted → no default; typed reads remain unaffected. |
| `display.statusProperty` | Enum property rendered as the status pill. Omitted → first enum property named with `status` or `stage`, otherwise no pill. |
...
The five fields above are optional compatibility controls: omission retains the
fallback stated in the table, while an explicit value survives
`catalog.export` → `catalog.apply`. `idPrefix` is effectively write-once —
changing it later affects new generated ids only; existing rows keep their old
prefix. Property prose is already authorable as
`properties.<name>.description`; it is projected to the legacy
`fieldDescriptions` map, so `fieldDescriptions` itself is not a separate
refinement key.

For the JSON/boolean property wire and storage behavior — including the
breaking change noted above and the raw-`TEXT` `sql_read` exception — see the
...
- **Display-only.** The label is a render-time substitution — the **stored value
  is never altered**. Where a value has no entry, the value falls back to the
  humanised raw token (`green` → `Green`) for compatibility, while the catalog
  audit warns so a Danish view cannot silently ship that fallback.
...
> Any enum value without a non-empty `valueLabels` entry falls back to a
> humanised raw token (`green` → `Green`) for compatibility. Add labels when
> the displayed vocabulary matters; a bound StateMachine's non-empty
> `StateMeta.label` is an equivalent label source.
...
| `via` | yes | Column name on the **target** ObjectType by default, or on the **source** when `fk_on: source`. Example: `contacts` link from customer → contact uses `via: customer_id` (the FK on contact). |
| `fk_on` | no | Which side owns `via`: `target` (default — parent→child) or `source` (a child→parent belongs-to link). |
| `on_delete` | no | Referential-action policy on target-side ownership links: `restrict` (default), `cascade`, `set_null`. Honoured by every `delete` path — auto-CRUD's `<obj>.delete`, declared handler `delete` steps, and direct `runtime.delete()`. Omit it on source-side belongs-to links (`fk_on: source`); declaration validation rejects it there. |
...
The `on_delete` field on a target-side ownership link controls what
`typedDelete` does when child rows exist. A source-side belongs-to link
(`fk_on: source`) is only for child→parent navigation and must not declare
`on_delete`; the declaration is rejected because typedDelete cannot enforce it:
...
into the compound machinery (docs/adr/0002). They're summarised here; the
...
## Effective-dated objects — `temporal`

Prices, standing orders, ratios — anything that is *in force from a date until
another date* — is a series of half-open periods `[from, to)` per key, with
exactly one open period (`to` empty) per key. Declare it once instead of
hand-writing the in-effect clause and the set/end verbs:

```yaml
action: catalog.refine_object_type
input:
  name: price
  temporal: { from: valid_from, to: valid_to, key: [product_id, customer_id] }
```

`from` and `to` are `string`/`timestamp` mapped properties (`to` nullable —
`NULL` means still open); `key` is a non-empty list of mapped properties that
identify the series (never `id`, never a bound). A `NULL` key member is a
legitimate value (a list price has no `customer_id`) and still collapses to
one open period. The declaration gives you:

- **`<type>.set`** — the period in effect from `from`. Closes the key's open
  period *at* the new `from` (the two abut, no gap, no overlap), inserts the
  new row, returns it plus `closed_id` (or `null`). Any period the new one
  would intersect — including one already given an end date — is refused with
  `VALIDATION` / `details.code: 'temporal_overlap'` naming `conflicting_id`.
  Neither names a period the caller may not read: the refusal then omits
  `conflicting_id` and the window, and `closed_id` is left out.
  `to` may be supplied for a bounded period; `to <= from` is
  `temporal_empty_period`.
- **`<type>.end`** — closes the open period of a key (or the row named by
  `id`) at `to`. `temporal_end_before_start` when `to` is not after the
  period's `from`; `NOT_FOUND` / `temporal_no_open_period` when nothing is
  open; `temporal_not_open` when the `id` names a closed period.
- **The schema guard** — a partial `UNIQUE` index on the key `WHERE to IS
  NULL`, created with the declaration (and on `catalog.apply` to a fresh
  org). Two writers racing for the same key cannot both open a period. A
  table that already holds two open periods for one key refuses the
  declaration (`temporal_open_rows_conflict`) so it never half-adopts.
- **`as_of` reads** — `canon objects list <type> --as-of 2026-04-01`, `objects
  get … --as-of`, `objects navigate … --as-of` (REST `?as_of=`, MCP `as_of`)
  return only the periods in effect on that date (`YYYY-MM-DD`, an ISO
  date-time, or `$now`); `get` reads `null` when the row is not in effect.
  On a type without `temporal` the read fails `VALIDATION` /
  `as_of_not_temporal`.
- **The `$as_of` filter token** — the same question inside any filter map: a
  ViewSpec `source.filter: { $as_of: '$now' }`, a handler step's
  `where: { product_id: '$input.product_id', $as_of: '$input.on' }`, or a
  REST `?where={"$as_of":"2026-04-01"}`. It compiles to
  `from <= D AND (to IS NULL OR to > D)` over the declared bounds.

Both verbs are generated like auto-CRUD: they appear in every surface, carry a
conformance fixture, and are **not** exported into `actions/` — the
declaration is the artifact. A hand-declared action with the same name
(`actions.declare price.set` with extra guards) replaces the generated one,
and survives re-refinement and reboot; the other verb stays generated. The
declaration round-trips through `catalog.export` → `catalog.apply` byte for
byte. Validated at declare/apply time (`temporal_invalid` names the bad
field).
...
This is the precise reason the worked `minimal-crm` scenario
...
input:
  ...
  defaults:
    ...
    replace: false  # suppress invoice.replace when invoice has part_of children
```
...
delete). The `replace:false` flag follows the same rule for a compound root;
on a non-compound type it is accepted as a forward-compatible no-op rather
than an authoring error.

---

## Optimistic concurrency — `expectedHash` / `force` on re-refine

A **re-refine of an already-refined type** is guarded by optimistic
concurrency on the refinement's content hash, default-on: omit both
`expectedHash` and `force` and the call is rejected before any write
(`CONFLICT/HASH_REQUIRED`, `details.current_hash`) rather than silently
last-writer-wins. A type refined for the **first time** has nothing to
conflict with, so the guard never fires there. Pass the hash read from
`catalog.export`'s ObjectType record as `expectedHash` (`canon schema
describe` does not currently surface `declaration_hash` — `catalog.export`
is the reliable read path for it); a mismatch is `CONFLICT/STALE_DECLARATION`
naming which top-level refinement keys drifted (e.g. `['properties','links']`).
`force: true` skips the check as a deliberate overwrite; supplying both
`expectedHash` and `force` is rejected as contradictory intent
(`VALIDATION/CONTRADICTORY_DECLARATION_INTENT`). See the base skill's *error
codes* reference for the full sub-code table.

```yaml
action: catalog.refine_object_type
input:
  name: lead
  expectedHash: "sha256:9f2c…"   # from catalog.export's prior refinement record
  properties:
    status:
      kind: mapped
      column: status
      type: enum
      values: [new, contacted, qualified, converted, disqualified, lost]
```
...
| Re-refine of an already-refined type, no `expectedHash`/`force` | `CONFLICT/HASH_REQUIRED` | Read the current `declaration_hash` and re-submit with `expectedHash`, or pass `force: true`. |
| `expectedHash` no longer matches the persisted refinement | `CONFLICT/STALE_DECLARATION` | Re-read the refinement, re-diff, re-submit with the fresh hash. |
...
- Phase 17.5 §6 — cascade-on-link semantics
- Phase 17.5 §7 — refinement contract
