...
> Any enum value without a non-empty `valueLabels` entry earns the advisory
> `elegance_missing_value_labels` — a chip that still shows the raw token
> (`green`) reads worse than one that shows "Healthy". A bound StateMachine's
> non-empty `StateMeta.label` satisfies the same contract. See
