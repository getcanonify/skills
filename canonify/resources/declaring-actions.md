...
Separately, the whole `platform` namespace is reserved outright — a name
that IS `platform` or starts with `platform.` (e.g. `platform.forge`) is
rejected at declare time with a `reserved_action_name` compile error. It's
held by the control-plane audit trail (ADR 0027), which tags every
privileged platform-side event with that prefix so `canon audit --origin
platform` can tell them apart from your own actions; a tenant action in
the namespace would let its rows read as control-plane events.
...
- `per-key` — when the caller supplies `idempotency_key`, the runtime
  caches the first envelope and replays it on retry, **for 7 days**; a
  same-key retry after that re-executes the action. Use this for
...
By default the action moves ONE row, read from an id-shaped input field
(`<type>_id` or `id`); without one, registration fails with `no_id_input`.
To move a set in one call, drop the id input and name the rows in the
handler with `transition` steps (usually in a `for_each` body) — the marker
is unchanged. See *Transitioning a set in one call* in
[`declaring-state-machines.md`](declaring-state-machines.md).
...
`packages/canon/src/handler-dsl/schema.ts`.
See the [Handler DSL cookbook](handler-dsl-cookbook.md) for shapes.
A block several handlers share is declared once with `fragments.declare`
and run with a `call` step (`{ "op": "call", "fragment": "<name>", "with":
{…}, "as": "<alias>" }`); declare the fragment before the actions that call
it — in a bundle it lives under `fragments/` and applies before `actions/`.
...
as the prominent inline button. This keeps a large custom-action surface
readable and gives the main workflow a clear CTA.
...
- `actions.describe <name> [--json]` — return the declaration JSON for
  ...
  transition, fixture, a `cli_example` field — a ready-to-paste
  `canon <name> --input '<fixture.args>'` invocation (derived from the
  declared fixture; `--input '{}'` when none) — and `declaration_hash`,
  a `sha256:<hex>` content hash of the persisted declaration (`null` for
  auto-CRUD actions with no persisted row). `--explain` on any per-action
  sugar command returns the same payload as a shortcut.
- `actions.update <declaration> [--expectedHash <hash>] [--force]` —
  replace an existing declaration. Same schema, same type-check, same
  persistence; differs from `actions.declare` in that it accepts a changed
  declaration where `declare` would reject with
  `INVALID_STATE/declaration_changed`. Guarded by optimistic concurrency
  (tick jqu) on the declaration's content hash, default-on: omit both
  `expectedHash` and `force` and the update is rejected before any write
  (`CONFLICT/HASH_REQUIRED`, `details.current_hash`) rather than silently
  last-writer-wins. Pass the hash read from `actions.describe` (or
  `catalog.export`) as `expectedHash`; a mismatch is
  `CONFLICT/STALE_DECLARATION` naming which top-level keys drifted. `force:
  true` skips the check as a deliberate overwrite; supplying both
  `expectedHash` and `force` is rejected as contradictory intent
  (`VALIDATION/CONTRADICTORY_DECLARATION_INTENT`). See the base skill's
  *error codes* reference for the full sub-code table.
...
- Phase 17.5 §5 — full spec
- `packages/canon/src/actions/actions/declare.ts` — the implementation
