...
### Transitioning a set in one call

The guard above reads ONE row, whose id comes from an id-shaped input field
(`order_id`, `id`, …). When one invocation must move many rows — lock every
open order of a delivery day, close every ticket of a project — the handler
names the rows instead, with a `transition` step (usually inside a
`for_each` body), and the marker stays exactly the same:
...
- A `transition` step writes nothing itself. It names a row (`object` must
  be the action's own ObjectType; `where` addresses one row, `NOT_FOUND` if
  it matches none). After the handler returns, the runtime guards **every**
  named row against `from`, writes `to` on each, and fires the hooks per row
  (`<audit_event_id>:hook:<seq>` keys keep counting across rows) — all in
  the parent transaction.
- One row outside `from` fails the whole invocation with `INVALID_STATE`
  (details name that row); nothing is written. Use a `when` on the step to
  decide membership instead, as above.
- Naming no row is not an error: the action succeeds and moves nothing.
  Naming the same row twice moves it once.
- The action needs **no** id input field; the input carries whatever the
  handler needs to find the set. Such an action cannot back an `automatic`
  transition (a schedule has one row id to hand over) — `state_machines.register`
  refuses that with `bad_schedule`.
- A `transition` step in an action without a `transition` marker, or naming
  another ObjectType, is refused at declare time with
  `VALIDATION/invalid_transition_step`.
...
If a column must stay freely writable (importer-owned values), do not bind
a machine to it — keep it a plain enum. A machine is a promise that the
column is transition-only. Bulk moves are not a reason to leave a column
unbound: declare a set transition (*Transitioning a set in one call*, above).
