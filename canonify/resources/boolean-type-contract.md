...
Review numeric columns with boolean-looking names—`is_`, `has_`, `can_`,
`at_risk`, `flag`, `active`—and retype them to `boolean` when they represent a
flag. This improves UI rendering (showing "Yes/No" instead of "0/1") and
clarity in read results.

**When you make that refinement:**
