...
| `elegance_missing_value_labels` | an enum property has one or more declared values without a non-empty `valueLabels` entry (or an equivalent bound state label), so a view can fall back to the raw token (`green`) | add `valueLabels` mapping every value to display text (the same site as `valueColors`), or label the bound state in its StateMachine |
