...
`packages/canon/src/runtime-types.ts` (`RuntimeError`).
...
### Two `NOT_FOUND` shapes: missing subject vs missing foreign reference

A `NOT_FOUND` names one of two different failures, and the `details`
discriminate them:

- **Missing subject** — the row the action operates on does not exist. The
  FSM transition guard reports it as `details: { object_type, id }` (the
  tick 532 contract above); a handler-DSL `read` / `update` / `delete` step
  reports it with `{ step, object, where }`. The runtime never pre-checks the
  action's own subject: the subject is addressed by convention as
  `<object_type>_id` or by an explicit `subject` whose `object_type` is the
  action's own, and whether it must pre-exist is the handler's call.
- **Missing foreign reference** — an input that names a row of ANOTHER
  ObjectType through a declared belongs-to link (`<link>_id` / the link's
  `via` column) or an explicit `subject` of another type. The runtime checks
  these BEFORE the handler runs, so an unknown foreign id never enters the
  transaction, and reports
  `details: { field, object_type, code: 'foreign_key_not_found', message_key: '@t:errors.foreign_key_not_found' }`
  with `message` set to the same `@t:` key (localisable by the org's message
  bundle). There is no `id` in this shape; `field` names the offending input.

Switch on `details.code === 'foreign_key_not_found'` to tell them apart. A
missing subject is never reported in the foreign-reference shape.
...
|---|---|---|---|
| `VALIDATION` | 2 | Schema decode of input failed; handler DSL type-check failed at `actions.declare`; handler writes the FSM-bound state column when a `transition` marker is set (`direct_state_write`); a `transition` step in a handler without a marker, or naming another ObjectType (`invalid_transition_step`); auto-CRUD detects a malformed declared property. | Call `canon actions describe <name> --json` (or `--explain`) to fetch the input JSON Schema; correct the offending field and retry. For `actions.declare` typecheck failures the `details.errors` array names the offending step and field. |
| `NOT_FOUND` | 3 | `read` step found zero rows for its `where` clause; the FSM transition guard found no row for the action's subject id; a declared belongs-to input (`<link>_id`, the link's `via` column, or an explicit `subject` of another ObjectType) named a row that doesn't exist — checked by the runtime before the handler runs, `details.code: 'foreign_key_not_found'` (see *Two `NOT_FOUND` shapes* above); `runtime.get(type, id)` was called with an unknown id; `runtime.invoke(name, …)` was called with an unregistered action name; `catalog.refine_object_type` targeted an ObjectType that isn't registered. | Verify the id exists via `canon objects get <type> <id>` or list the candidates via `canon objects list <type>`. For a `foreign_key_not_found`, `details.field` names the input to fix. For unregistered actions, `canon actions list` is the discovery surface. |
...
(docs/adr/0002 §5, wiki decision 0009).
...
### Effective-dated (`temporal`) sub-codes

Generated `<type>.set` / `<type>.end` and `as_of` reads refuse with a
switchable `details.code`:

| Code | Kind | Where | Meaning / fix |
|---|---|---|---|
| `temporal_overlap` | `VALIDATION` | `<type>.set` | The new period intersects an existing one (open or already closed) of the same key; `details.conflicting_id` / `conflicting_from` / `conflicting_to` name it. End or re-date the existing period first. |
| `temporal_empty_period` | `VALIDATION` | `<type>.set` | `to` is not after `from` — periods are half-open `[from, to)` and cannot be empty. |
| `temporal_end_before_start` | `VALIDATION` | `<type>.end` | `to` is not after the open period's `from`. |
| `temporal_not_open` | `VALIDATION` | `<type>.end` | The `id` names a period that already has an end date; only the open period of a key can be ended. |
| `temporal_key_required` | `VALIDATION` | `<type>.end` | Neither `id` nor the full key was supplied; `details.missing` lists the key properties. |
| `temporal_no_open_period` | `NOT_FOUND` | `<type>.end` | Nothing is open for the key. |
| `temporal_invalid` | `VALIDATION` | `catalog.refine_object_type` / `catalog.apply` | The declaration names a bound or key member that is not a mapped `string`/`timestamp` property, repeats one, or includes `id`; the message names the field. |
| `temporal_open_rows_conflict` | `VALIDATION` | `catalog.refine_object_type` / `catalog.apply` | The table already holds two open periods for one key, so the one-open-period index cannot be created; close the extra rows, then re-declare. |
| `as_of_not_temporal` | `VALIDATION` | `as_of` reads, `$as_of` filters, handler typecheck | The ObjectType has no `temporal` declaration. |
| `as_of_invalid` | `VALIDATION` | `as_of` reads, `$as_of` filters | The value is not a `YYYY-MM-DD` date, an ISO date-time, `$now`, or a relative-date token. |

### `catalog.apply` pre-write validation details

These diagnostics ride the top-level `VALIDATION` error rather than the
`CONFLICT` family. They run before the first apply sub-step writes, so
`details.apply_state` is `"pre_write"` and no partial-durable convergence note
is added.

| Discriminator | Where | Meaning & recovery |
|---|---|---|
| `details.code: 'registry_content_mismatch'` | `catalog.apply` | An action handler references an ObjectType property whose live registry type differs from the type derived from the bundle's schema DDL. `canon catalog test` would type-check that handler against a different type than apply does. The message and details name the action, ObjectType, property, `live_type`, and `bundle_type`. Only properties the handler actually addresses are compared, and properties the bundle re-declares in an ObjectType refinement are excluded (apply publishes those first). Fix by converging the live column with a migration, or by restating the property in the bundle's ObjectType refinement, then re-apply. `force: true` does **not** override this fence — it bypasses only the optimistic baseline drift check. |
...
- **`error`** — a real defect that would render broken or apply an incoherent
  ...
  `invalid_currency`, `invalid_composite_filter`, `invalid_aggregate_compare`,
...
- **`warning`** — a report-only nudge. Warnings **do not block** a normal apply;
  ...
  and `missing_part_of`. One warning is exempt from the `strict` promotion —
  see `removed_property_not_dropped` below.
...
child, `dimension` requires an **`enum`**, `date` requires a **`string`** or
**`timestamp`**, and `reference` requires the terminal to be the **`via`**
foreign key of a declared to-one link. Link paths are resolved through the same
closed grammar as other derived reads. The message names the view and the
offending property. Point the filter at a real child-source property/path of the
matching type, or remove the filter. See the viewspecs skill's
*Reader-adjustable filter bar*.

### `invalid_aggregate_compare` / `invalid_aggregate_window` / `invalid_aggregate_compare_offset` (error)

The **aggregate envelope** — `source` + `compare` + `window` — is shared by the
`metric`, `chart` and grouped `table` ViewSpec kinds, and these three codes are
its rules, whichever kind declares it. `invalid_aggregate_window` fires when
`window` is declared without an aggregate source or without a time-bucketed
`groupBy` spine (a window is a count of trailing buckets). `invalid_aggregate_compare`
fires when `compare` lacks that spine, sits beside `seriesBy`, is declared over
a non-additive op (`avg`/`max`), or when `source.filter` constrains the spine's
own property (which would cut the widened scan the layers derive from — scope a
compared read with `window` instead). `invalid_aggregate_compare_offset` fires
when `compare.offset.period` is not admitted by the spine's bucket. The message
names the rule; the fix is the spine, the op, or the layer. See the viewspecs
skill's *`compare`* and *`window`* sections.
...
- A policy naming a column that the **same bundle** adds — a new property on
  an existing ObjectType, declared alongside the policy that filters on it —
  resolves correctly: the check reads the bundle's own declared shape, not
  only what's live. A column neither live nor bundle-declared still fails.
...
a top-level `VALIDATION` failure (checked before any write commits), and
`details.apply_state` is `"pre_write"` — the message carries no
re-apply-to-converge note, since nothing was written yet (see the
app-authoring skill's *apply is fully declarative* atomicity callout). Via
`canon catalog validate` it's one entry in the `diagnostics` array with this
code.

**The pre-flight resolves columns against what the bundle DECLARES, not just
what's live.** A `row_filter` naming a column your bundle is *adding in this
same apply* (a new property on an existing ObjectType, alongside a policy
that filters on it) is checked against the bundle's own declared shape, not
only the pre-apply registry (tick `h7z`) — so "add column `principal_id`, and
a policy filtering on `principal_id`" in one bundle applies cleanly instead of
false-rejecting with an unknown-column error on every re-apply. A column
neither the live type nor the bundle declares still fails here, before any
write — this only widens what counts as "known," it doesn't loosen the
compile check itself. Because this failure fires before the first sub-step
write, its `details.apply_state` is `"pre_write"`, not `"partial_durable"` —
see *apply is fully declarative* in the app-authoring skill.
...
### Coherence audit diagnostics

`canon catalog audit <dir>` runs the three retained coherence rules from
`packages/canon/src/catalog-audit.ts` and groups them with structural
validation diagnostics. `C1-shadow-entity` and `C11-wildcard-grant` are in
`BLOCKING_AUDIT_CODES` and reject `catalog.apply`; `C10-float-money` remains an
error-level authoring-loop diagnostic, intentionally outside the blocking set
until a first-class `money` type exists. The CLI audit command and the
domain-modelling skill are its named consumers. The rule table and blocking set
are covered together by the catalog audit tests.
...
- Phase 17.6 §7 — CLI exit-code table
- Phase 17.5 §4 — how the executor raises these kinds from DSL steps
- Phase 18.5 — FSM guard semantics and the `current_state` / `allowed_from` detail shape
