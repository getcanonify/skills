...
If it decodes here it executes inside the runtime; the platform's CI
enforces this by decoding the first JSON block on this page against
`HandlerDsl`.
...
The canonical multi-table atomic write, lifted verbatim from the
platform's worked `minimal-crm` scenario.
...
the worked `minimal-crm` scenario are all this pattern.
