...
`canonify-domain-modelling`, `canonify-app-lifecycle`,
`canonify-viewspecs`, `canonify-scenarios`,
`canonify-isomorphism`, and `canonify-esign-sender`)
ship in the same `@canonify/skills` package.
...
By default these write to **`.agents/skills/`**, the one project skill root
read by agents in the repo; `.claude/skills` is only a compatibility alias.
...
|---|---|---|
| `schema.propose_migration` | Create or alter tables. Auto-CRUD synthesises a default ObjectType + the five ordinary CRUD ActionTypes from the new table; a compound root adds `<table>.replace` after a `part_of` link is declared. | [resources/declaring-tables.md](resources/declaring-tables.md) |
...
### Four obligations, before you declare anything

The four verbs above are **mechanics** — what you *can* declare. These four
rules are what you *must*. They are not style preferences; each one is a defect
class the platform's coherence audit reports at `error` severity, and three of
them are effectively unfixable once real data has accumulated on top.

1. **Reuse the canonical object.** A property whose value names a real-world
   entity that already has an ObjectType is a defect. Declare a link and expose
   the display value as a `kind: "derived"` property through it — never a mapped
   text column. Retyping a customer's name into `proposal.customer_name` makes
   the record's identity a free-text string that nothing can navigate,
   deduplicate or keep in sync. (`C1-shadow-entity`)
2. **Anchor every child.** If an object has an FK column naming another
   ObjectType, it MUST declare the upward link
   (`{ target, cardinality: one, via: <fk>, fk_on: source }`). Without it you
   cannot navigate a child to its owner, `on_delete` has nothing to attach to,
   and no record page can scope a child table to the record it is showing.
   Downward `many` links are not a substitute; the upward link is what makes
   navigation and ownership explicit.
3. **Reuse the platform primitive.** Before modelling a capability, run
   `canon objects list-types` and `canon actions list --json` **in a fresh org**
   and read what the platform already ships — e-signature, `type: 'file'`
   storage, the audit trail, the outbox, approvals, idempotency, state machines.
   Building a second implementation is a defect even when yours works.
   Treat the platform catalog as the source of truth before adding a duplicate
   capability.
4. **Effective-date time, snapshot money.** A value that changes over time and
   matters financially is an **append-only, effective-dated row**
   (`valid_from` / `valid_to`) — never a mutable column. Never update a price;
   close the interval and insert a new one. Money is **integer minor units**
   with a currency beside it, never a float. (`C10-float-money`) A settled
   transaction line **snapshots** the resolved amount *and* references the row
   it resolved from. A row with `0` is *deliberately free* and is not the same
   as no row. Periodic reports you hand to someone are stored, sealed objects —
   not live queries; corrections land as adjustment lines on the next report.

Each rule, with a worked before/after and the full pricing example, is in the
[`canonify-domain-modelling`](../canonify-domain-modelling/SKILL.md) skill.
Read it before your first `schema.propose_migration`.

The `canonify-scenarios` skill ships worked `session.yaml` fragments
demonstrating all four verbs in context (see its
`resources/scenario-patterns.md`).
...
Policies on target-side ownership links: `restrict` (default — `INVALID_STATE`
if children exist), `cascade` (delete children first), `set_null` (null-out the
FK). Honoured by **every** delete path, not just declared handlers. A
source-side belongs-to link (`fk_on: source`) is navigational and must omit
`on_delete`; declaration validation rejects it because typedDelete cannot
enforce a policy on that edge.
...
and Phase 18.5 (platform spec) for the cross-validation rules.
...
SQL — see Phase 17.5 §1 (platform spec) for the deliberate omission
list.
...
`packages/canon/src/runtime-invoke.ts`):
...
13. **Idempotency cache stored.** If `idempotency_key` was supplied,
    the envelope is cached for future replays — **for 7 days**. After that
    the cached envelope is pruned and a same-key retry RE-EXECUTES the
    action rather than replaying it.

Retention windows for the other durable trails: `_audit_event` rows are
kept for **90 days**, and `_outbox` event rows for **14 days** — plan
audits and forensics within those windows.
...
- **`_audit_event`**: one row per call, written outside the business
  tx. Columns: `id` (the `audit_event_id` returned in the envelope),
  `action_id`, `principal_id`, `principal_type`, `args`, `result`,
  `status`, `error` (on failure), `started_at`, `completed_at`, and
  `knowledge_used` (a JSON array of cited Knowledge ids). There is no
  `org_id` column — each org is its own database.
...
Every write/mutation invocation — success, denial, or error — writes an
`_audit_event` row (Phase 17.5). Reads are audited on **denial only**
(perf decision 2026-07-07): a granted read returns an empty
`audit_event_id` (`''`) and leaves no row in the trail. To see what
you've done recently, including denials, failures, durations, and the
principal each call ran as, use:
...
envelope is no longer in scrollback. The trail is an ObjectType, so the
same rows come back from `canon objects list _audit_event` (and from
`GET /v1/objects/_audit_event` on REST, or the equivalent MCP read) —
`canon audit` just compiles its flags into that one call.

**What you see depends on your role.** An audit row carries the result
of the call it records, on whatever object type that call touched — so
the trail is fenced by the reader's authority, not by each row's own
type. On this read path an **admin** or **owner** reads the whole org
trail, while a **member** (and a read-scope OAuth client) reads only the
rows its *own* calls wrote. An empty page as a member means "you did
nothing in that window", not "the org was idle". A hosted app's end users
and a public app's visitors never read it: `_audit_event` is a system
ObjectType, like the member directory (`org_member`) and the e-signature
types, which live in system tables. Publishing an `access_manifest` that
names a `_`-prefixed type, or grants a read of any system type, is refused
with `VALIDATION`, and a broader grant (`resource: {}`) does not reach one.
The same holds through an action: granting invoke on a platform verb over a
system table (`org_member.list`) is refused, and so is an action of your own that
touches one when an app's end user or a public visitor invokes it — whether it
reads it in a handler step, deletes a row whose link CASCADES into it, or names
it as a calendar or a foreign-key subject. A carrier also cannot invoke an action
whose `on_fail` or state-machine hook would queue work for the platform to run
later, unless that queued action is one the platform marks reaction-safe. Your
org's members still run all of it — this is a fence on who the app serves, not on
the action. Have a portal-facing action return only what the portal needs, from
your own object types.

> `sql_read` keeps the same fence. It cannot filter rows, so it refuses any
> statement that reaches the trail when your read of it is fenced — the table,
> its view, or a join or subquery onto either. The same rule covers every
> table: your read policy decides each table a `sql_read` statement reaches,
> and a table whose read is denied, row-filtered or column-masked for you
> refuses the whole statement, while the typed read applies the narrowing.
> Tables the **platform** owns — anything no ObjectType backs, a table holding
> columns no ObjectType exposes, and `sqlite_schema` — need the org **owner**
> or **admin** role on top of that, because such a table can hold data of any
> type and no single type's grant covers it. A broad `resource: {}` read grant
> does not substitute for the role.
> Every `sql_read` call is itself recorded in the trail.

#### Control-plane events in the same trail

The trail is not only your actions. Privileged **control-plane**
operations — API-key mint and revoke, member role changes, org
rename/delete/leave/export/restore, hosted-app and form publish,
hosted-app token mints — write into the same log, with an `action` under
the reserved `platform.` prefix (`platform.key.revoke`,
`platform.user.update_role`). They are listed alongside your Canon events
by default, newest first, and the prefix is what tells them apart — it is
reserved, so no declared action of yours can ever land in it.

`--origin` filters by that prefix:

```sh
canon audit --origin platform            # control-plane events only
canon audit --origin canon               # your actions (and denied reads) only
canon audit --origin all                 # both (the default)
canon audit --origin platform --status denied --since 7d
```

The filter is applied server-side, so `--limit` and the cursor page over
the filtered set.

> ⚠️ **`--origin platform` shows org-scoped control-plane events only.**
> Authentication and session events — sign-in, select-org,
> switch-org, `cli/authorize`, `agent/exchange`, magic-link, the device
> flow, apps-SSO, and signup — are **not** listed by `canon audit`, in any
> mode. (Sign-OUT is org-routed and IS listed, as
> `platform.session.revoke` — it is the one session event you can see here.) Those are recorded cross-tenant in the control-plane database,
> which is operator-only: serving them through a per-org command would
> disclose one tenant's sign-ins to another. An empty `--origin platform`
> page is **not** evidence that nobody authenticated. If you need a
> sign-in trail, ask a Canonify operator.
...
Run `canon --help` for the full daily-use command surface.
...
|---|---|---|
| **REST** | `GET /v1/actions`, `GET /v1/schema_describe` | `POST /v1/actions/<name>` with `{ input, idempotency_key? }` body; `Authorization: Bearer <api_key>` or `Session <token>`; `X-Principal-Json` for testing. |
...
The `minimal-crm` scenario (maintained in the Canonify platform repo)
...
- [`canonify-domain-modelling`](../canonify-domain-modelling/SKILL.md) — the four
  obligations above in full: the shadow-entity before/after, anchoring, checking
  for platform primitives, and effective-dated pricing worked end to end.
- [`canonify-objects-actions`](../canonify-objects-actions/SKILL.md) — the
  ObjectType/ActionType mechanics in depth.
- [`canonify-app-authoring`](../canonify-app-authoring/SKILL.md) — the
  five-chapter tutorial from an empty org to a working app.

## Source-of-truth references

- Phase 17.5 — Declarative Handlers (platform spec) — the spec this manual implements
- Phase 17.6 — CLI Ergonomics (platform spec) — surfaces, the skill REST routes, exit-code contract
- Phase 18.5 — StateMachines (platform spec) — FSM frame
- `packages/canon/src/handler-dsl/schema.ts` (platform repo) — the effect/Schema your handler JSON parses against
- `packages/canon/src/runtime-invoke.ts` (platform repo) — the orchestrator
