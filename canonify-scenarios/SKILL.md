...
- **`canon catalog test` shares ONE Canon across every scenario in the run**
  (unlike `bun test tests/scenarios/`, where each scenario boots its own).
...
- [`tests/scenarios/_harness/apply-and-run.ts`](../../../tests/scenarios/_harness/apply-and-run.ts) — the `canon catalog test` ephemeral CI-gate loop: one apply, shared-vs-`--fresh-per-scenario` Canon lifetime, the id-collision naming diagnostic.
