...
2. **Fetch the full stored envelope for that call:**
   ...
   ```
   `canon audit <id>` is sugar over `canon objects get _audit_event <id>` — the
   trail is the read-only `_audit_event` ObjectType — and its `result` field
   is the exact envelope data the runtime produced for the CLI call.
...
3. **Do the same over REST**, then fetch its stored envelope:
   ...
   curl $BASE/v1/objects/_audit_event/<audit_event_id> -H "authorization: Bearer $KEY"
   ```
   `GET /v1/objects/_audit_event/<id>` (the typed read backing
   `canon audit <id>`) returns the same row under `data`.
