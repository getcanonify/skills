...
interface Envelope<T = unknown> {
  ...
  audit_event_id: string;          // non-empty on writes; empty ('') on granted
                                   // reads — reads audit on denial only
                                   // (perf decision 2026-07-07)
...
The runtime overlay (`packages/canon/src/runtime-types.ts`, re-exported from `runtime.ts`) adds the error field:
...
interface RuntimeError {
  ...
  field_errors?: Record<string, string>;   // per-field message map (see below)
  failed_precondition?: { field?: string; field_path?: string };
...
## Structured per-field errors: `field_errors` and `failed_precondition`

Two **additive** error fields (absent on every pre-existing envelope shape) let
a client attach an error message to the specific input field that caused it,
instead of only rendering a top-level banner. They ride the same `error`
object every surface returns, so they need no per-transport handling:

- **`field_errors`** — `{ <field name>: <message> }`. Populated automatically
  whenever a `VALIDATION` failure comes from input-schema decoding (folded from
  `issues`), and also whenever a handler raises a domain error whose
  `details.field_errors` carries a map — the runtime lifts it onto this
  first-class slot.
- **`failed_precondition`** — `{ field?, field_path? }`. Set when a
  handler-declared precondition check (an `assert` naming a field) fails, so
  the client can attach the author's rejection message to that named field
  instead of a generic banner.

Both are populated by the **runtime**, before the envelope reaches any surface
adapter, so REST, CLI (`--output json`), and MCP all carry the identical
`field_errors` / `failed_precondition` shape for the same failure — there is no
surface-specific formatting to keep in sync. The CLI's default **human-readable**
text output additionally renders per-field `issues` inline on a `VALIDATION`
failure (instead of a bare `Runtime error: …` line) — that is a CLI-only text
convenience on top of the same JSON envelope, not a divergence in the envelope
contract: `--output json` on every surface still returns the one shape above.
...
- **Stale-version compound write** (optimistic CAS): a `ConflictError` whose
  ...
  re-read and retry.
- **`catalog.apply` stale canon**: you pinned a `baseline` and an artifact your
  apply would change or prune has drifted since. `details` is
  `{ code: 'STALE_CANON', drifted: [{ key, current, baseline }] }` (per-artifact;
  a re-provisioned DB carries `reason: 'epoch_mismatch'`) — re-export the canon,
  re-diff, re-plan, re-apply. Disjoint edits pass.
- **`catalog.apply` already running**: another apply holds the per-org lease.
  `details` is `{ code: 'APPLY_IN_PROGRESS' }` — kept distinct from
  `STALE_CANON` so a client can tell "retry shortly" from "re-read first".
