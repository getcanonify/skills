...
sql_read(statement, principal, args?) → envelope  (read-only analytical escape hatch)
```
...
meta-tool `actions.invoke`; its wire shape is `{ name, args, principal }` only
on the in-process mount the conformance runner uses — the production `/mcp`
lane is a **remote** mount that drops `principal` from the published tool
schema entirely and binds the server-resolved caller instead (see "Reaching
MCP" below and `resources/runtime-contract.md`). All three decode their wire
format into the same `(action_id, args, principal)` triple and call `invoke`.
See `resources/runtime-contract.md` for the full mapping of each surface onto
each entry point.
...
A bare `https://<host>/mcp` (no slug) also works, for backward compatibility
and for API-key / agent-session callers: the org is resolved from the
credential instead of the URL. OAuth is **not** admitted on the bare form —
its resource-binding check needs the URL's org — so an interactive client
always uses the slugged `/mcp/<org-slug>` URL.
...
- **API key** — the programmatic path. **Agents and service accounts keep using
  ...
  principals. A key inherits its creator's full org role by default; mint a
  narrower one with `canon key create --type agent --name <name> --role
  member|admin` (never `owner` — that's not a grantable key role) when the
  principal shouldn't get everything the creator can do. The role is **frozen
  on the key**, not re-derived from the creator, so it doesn't silently widen
  or narrow later if the creator's own membership changes. Omit `--role` for
  the legacy behavior (full creator-role inheritance).
...
```ts
{
  ...
  audit_event_id,    // empty ('') on granted reads — reads audit on denial only
  ...
  error?,            // { code, message, issues, field_errors?, failed_precondition?, details? } on failure only
}
```
...
(`VALIDATION`, `NOT_FOUND`, `FORBIDDEN`, `INVALID_STATE`, `CONFLICT`,
`INTERNAL_ERROR`). A `VALIDATION` (or handler-raised) error may additionally
carry `field_errors` — a `{ <field>: <message> }` map for attaching the error
to the offending input field — and/or `failed_precondition: { field }` for a
named precondition check. Both are additive and identical across REST, CLI,
and MCP; see `resources/envelope-shape-and-error-handling.md`.
...
Reads are audited on **denial only** (perf decision 2026-07-07): a granted
read returns an empty `audit_event_id` (`''`) — there is no row to look up.
Writes/mutations are fully audited.
...
3. Do the same via REST (`POST /v1/actions/<name>`), grab its
   `audit_event_id`, and fetch it via `GET /v1/objects/_audit_event/<id>` —
   the trail is the read-only `_audit_event` ObjectType.
