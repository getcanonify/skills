...
|---|---|---|---|
| 1 | `schema.propose_migration` | Tables. Auto-CRUD gives you a default ObjectType + five ordinary CRUD actions for free; compound roots additionally get `replace` after a `part_of` link. | [schema-bootstrap.md](resources/schema-bootstrap.md) |
...
### One approval path for agent-authored declarations

There is one approval queue for both business invocations and catalog
declarations. When an agent invokes `actions.declare`,
`catalog.refine_object_type`, or `view_specs.declare`, the declaration action's
`requires_approval_when: "$principal.kind = 'agent'"` governance predicate
returns an envelope with `approval_status: 'pending'` and a `request_id`; the
declaration is not materialized yet. An owner (or another principal with the
`manage` grant) approves it with `actions.approve` and `{ "request_id": "..." }`,
or rejects it with `actions.reject`. Approval re-dispatches the original
declaration through the normal runtime, so the ActionType, refinement, or
ViewSpec is validated, persisted, and published on the same path.

Do not look for `actions.propose`, `objects.propose`, or `viewspec.propose`, and
do not read `_proposals`: those were the retired second approval track. The
durable pending record is `_approval_requests` for every approval-required
invocation.
...
Over REST that is `GET /v1/actions` and
`GET /v1/schema_describe`. Over MCP it is the `actions.list` tool. All
three return the same catalog.

Run the same three commands in a **fresh, empty org** at least once, and read
the result carefully: what comes back there is the **platform** surface —
e-signature (`signature_envelope` and friends), `type: 'file'` storage, the
audit trail, the outbox, approvals, idempotency, state machines. Those are
capabilities you get for free, and building a second implementation of one is a
defect even when yours works. So this section means two things, not one: *what
has my org already built?* **and** *what does the platform already ship?*

> **Before Chapter 1, read
> [`canonify-domain-modelling`](../canonify-domain-modelling/SKILL.md).** This
> tutorial teaches the mechanics of each verb; that skill states the four
> obligations those mechanics must satisfy — reuse the canonical object rather
> than retyping it into text columns, declare the upward link over every FK,
> reuse the platform primitive, and model time and money as effective-dated
> append-only rows with snapshotted settlements. Three of the four are
> effectively unfixable once real data has piled up on top, so they belong
> before your first migration, not after your first month-end.
...
the five ordinary CRUD ActionTypes (`<table>.create`, `.read`, `.update`,
`.delete`, `.list`) from the table shape — so a single migration already
gives you a working, queryable data model on every surface. A compound root
also gets `<table>.replace` once a `part_of` child link is declared.
...
**Input shape:** `{ name: string, description?: string, confidential?: boolean, idPrefix?: string, properties?: {...}, links?: {...}, defaults?: {...}, display?: { title?, subtitle?, formFields?, primaryProperties?, searchable?, defaultFilter?, statusProperty?, actions? }, browsable?: boolean, rollups?: {...}, parts_frozen_when?: {...}, temporal?: { from, to, key } }`
...
For paths, expand, measures, lookups, and reader filters, follow the complete
grammar in the public [`canonify-viewspecs` skill](../canonify-viewspecs/SKILL.md):
[binding and source paths](../canonify-viewspecs/resources/viewspec-binding-and-sources.md),
[analytics and the PortionPlan](../canonify-viewspecs/resources/viewspec-analytics.md),
and [composition filters with `$context`](../canonify-viewspecs/resources/viewspec-composition-patterns.md).
...
| `composite` | A container of nested specs — `layout` selects `stack` (default), `grid`, or `tabs`; `filters` adds `date-range`, `dimension`, `reference`, or `date` reader controls |
| `metric` | One KPI tile — a scalar aggregate, or a time-bucketed `groupBy` spine with `window` (a sparkline) and `compare` (a delta badge), read in one request |
| `chart` | A bar or line chart of one measure across one dimension (an aggregate **with** `groupBy`), optionally split into series with `seriesBy` |
...
aggregate; `sum`/`avg`/`max` take a numeric `:property` argument
...
For an aggregate source, `groupBy` and `filter` accept the same closed path:
a direct property, one or two to-one links, and a typed terminal. A to-many
filter path fails with `link_path_to_many`; reserve the one to-many fan-out for
`expand`. When the result is a product, use a labeled `aggregate.of` with one
to three numeric factors and at most one correlated `lookup`. Every traversed
ObjectType, including a lookup type, is policy-gated like a direct read.
...
### You shipped a SLICE, not an app

Read that heading literally and it is a trap. What you have is one domain, one
table set, one nav — a **slice**. The five verbs above are cheap to repeat, and
an agent handed a large brief repeats them across every domain in it and ends
up with many prototypes and no product. Before you start the next domain, read
[`canonify-app-lifecycle`](../canonify-app-lifecycle/SKILL.md): it defines what
a shippable slice is, when to stop adding surface, what to do with unfinished
work, and the retained coherence checks you clear before calling anything done.
...
**Re-declaring an existing slug is guarded.** `apps.declare` also takes
`expectedHash?` / `force?`. A re-declare of an EXISTING `slug` is guarded by
optimistic concurrency, default-on: omit both and the call is rejected before
any write (`CONFLICT/HASH_REQUIRED`) rather than silently last-writer-wins —
the FIRST declare of a fresh slug has nothing to conflict with, so this never
fires there (the two `crm` examples above are illustrative snapshots, not a
literal replay — re-running the second against a live `crm` app needs the
hash from the first, read via `catalog.export`, or `force: true`). A mismatch
is `CONFLICT/STALE_DECLARATION` naming the drifted keys. Same contract as
`actions.update` — see the base skill's *error codes* reference.
...
```sh
canon catalog apply ./my-app-canon --baseline <token>   # reconcile a canon to the bundle
```

(Every apply carries either `--baseline <token>` or `--force` — the
concurrency guard is default-on; see *Optimistic concurrency* below.)
...
**"Reconcile" means your DECLARATIONS — never the physical table.** `apply`
never diffs or `ALTER`s a live table. The bundle's `schema/` section exists to
**bootstrap tables that do not exist yet** (each entry is a full `CREATE TABLE`,
and an "already exists" error is tolerated as success), so on an existing table
that step is a no-op no matter what the bundle says.

So adding a property mapped to a **new column on an existing table** is a
**two-step** change — migrate first, then apply:

```sh
# 1. add the column (out-of-band DDL — apply will NOT do this for you)
canon actions invoke schema.propose_migration --input - <<'JSON'
{
  "description": "staff_member.employment_type — full-time vs part-time",
  "sql": "ALTER TABLE staff_member ADD COLUMN employment_type TEXT"
}
JSON

# 2. the column now exists, so the property declaration applies
canon catalog apply ./my-app-canon
```

Skip step 1 and `apply` fails with `Property "employment_type" maps to column
"employment_type" which does not exist on table "staff_member"` — the
declaration is refused because the column it names is not there. That is the
"unknown column" case above doing its job, not a reconcile that fell short.
...
- **Closed shapes — unknown fields are REJECTED, not ignored.** Every artifact
  ...
  - A `composite` scopes a child list to the parent row through the child's own
    **`source.filter`** (e.g. `{ customer_id: "$context.id" }`), not a field on
    the composite. Its optional `filters` row adds reader overlays with
    `date-range`, `dimension`, `reference`, or `date`; full path and
    `$context` rules are in the linked ViewSpec references above.
...
- **Live registry-content parity.** `canon catalog test` type-checks your
  handlers against an ephemeral canon built from the bundle; `apply` type-checks
  them against the LIVE canon. Before apply type-checks any handler, it compares
  the two views for every ObjectType property that handler addresses, and fails
  with `registry_content_mismatch` naming the action, the property, the live type
  and the bundle type — instead of handing you a type error you cannot reproduce
  locally. Two deliberate narrowings keep it quiet: only properties a handler
  actually addresses are compared (unrelated catalog drift never blocks an
  apply), and a property your bundle re-declares in an ObjectType refinement is
  excluded, because apply publishes refinements *before* it type-checks actions —
  so changing a property's type and its handler in one bundle still applies
  cleanly. What is left is the skew apply cannot converge, because apply never
  ALTERs a live table: a live column whose type drifted from your DDL, an organic
  live refinement your bundle does not restate, or a property the live ObjectType
  has lost. This is a **pre-write** `VALIDATION` failure (`details.apply_state` is
  `"pre_write"`), so nothing from the bundle is durable yet.
...
| `unknown_column_ref` (& `source_object_type_mismatch`, `invalid_board_group_by`, `unknown_action`, `invalid_column_format`, `unsupported_sortable_column_ref`, …) | A table/detail/board/panel/button view's column / property / groupBy / filter / sort / action ref doesn't resolve — the declaration-layer checks `validate` now replays (tick `6cu`). `unsupported_sortable_column_ref` specifically: a link-traversal column marked `sortable: true` — relation-path sorting is not supported by design because `runtime.list` only sorts source-object properties and does not perform relation joins or per-row reads; denormalize the value or sort by a local source property. |
...
`catalog validate` runs the same registry-content comparison against the same
live registry, so you can see the skew before you apply — run it against the
target (`canon catalog validate ./my-app-canon --live <env>`). `apply` stays the
authority: it re-reads the live registry at the write boundary, so a canon that
changed in between is caught there and nowhere else.

### apply is fully declarative — it prunes (ticket 95j)

`apply` reconciles the live **`managed` definition partition** to **exactly** what
the bundle declares. Physical schema tables are the exception: `apply` only
bootstraps tables that do not exist and never diffs or `ALTER`s an existing table.
When a bundled `CREATE TABLE` differs from the live DDL, apply emits the advisory
`schema_ddl_divergence` diagnostic; use `schema.propose_migration` for deliberate
physical schema changes, or `--strict` to fail before any apply write.

It is **not** additive: a managed artifact that was present in
...
**Dropping a property from a refinement does NOT drop the column.** The prune
above is whole-artifact (a refinement/action/view disappears entirely). A
single **property** removed from `catalog.refine_object_type`'s `properties`
map is narrower: the stored declaration updates, but the physical column
stays — auto-CRUD re-infers the property from it on every boot — so a live
org silently diverges from a fresh org built off the same bundle. `apply` now
emits a `removed_property_not_dropped` warning naming the persisting column
and the `schema.propose_migration` (`DROP COLUMN`) path to actually converge;
the warning never blocks apply, not even under `strict`. See the base skill's
*error codes* reference.

> Atomicity is **per-substep**, not whole-apply: a mid-apply failure can leave a
> partial state (the steps that already committed stay committed) — safe
> because every step is upsert / reconcile-in-place, so a clean re-apply
> converges. A `VALIDATION` failure that escapes the write region says so
> explicitly: the error message appends this convergence note and
> `details.apply_state` is `"partial_durable"`, so a bare substep error reads
> as "fix and re-apply", not "the org is broken."
>
> Pre-write `VALIDATION` failures are exempt from that decoration: the
> declare-time access-policy check (`invalid_access_policy`) and the live
> registry-content parity check (`registry_content_mismatch`) run before any
> sub-step write, so nothing is durable yet and re-applying converges nothing.
> Their `details.apply_state` is `"pre_write"`, and the message carries no
> re-apply-to-converge note — just the real problem to fix. This mirrors
> `STALE_CANON`'s pre-write posture above.

### Optimistic concurrency — `--baseline` / `--force` (ADR 0031)

The guard is **per-artifact**. `catalog export` prints a **baseline token** on
stdout — an opaque fingerprint of every artifact's content at export time (it is
deliberately **not** written into the on-disk `manifest.json`, so bundles stay
diffable). The check is **default-on**: a bare `canon catalog apply <dir>`
against a live canon is **refused before any write** with `BASELINE_REQUIRED`
(the shared org is the normal case — several authors and agents apply against
the same canon, and last-writer-wins silently loses updates). Every apply must
say which of two things it means:

```sh
canon catalog apply ./my-app-canon --baseline <token>   # pin: apply only if nothing you touch has drifted
canon catalog apply ./my-app-canon --force              # deliberate overwrite, no baseline drift check
```

At apply, the canon compares your baseline against the current per-artifact
content and, for every artifact your reconcile would **change or prune**, checks
whether it drifted since you exported. If any did, apply is **rejected before
any write** with a `STALE_CANON` 409 whose `details.drifted` names exactly which
artifacts moved (`action:<name>`, `view:<name>`, …) — re-export, re-diff, re-plan
against those, re-apply. **Disjoint edits pass**: if a teammate edited a
*different* artifact than yours, your apply still lands — only same-artifact
races conflict. A concurrent apply already in flight is a distinct signal —
`APPLY_IN_PROGRESS` (retry shortly), not stale. A baseline taken against a
since-re-provisioned database carries `details.reason: 'epoch_mismatch'`.

**The check is default-on — apply does NOT proceed unconditionally.** Omitting
both `--baseline` and `--force` is itself rejected before any write
(`BASELINE_REQUIRED`), because a blind apply is exactly how a stale bundle
silently reverts someone else's work. What happens next depends on the terminal:

- **Non-interactive** (CI, an agent, anything piped/redirected) fails closed
  immediately — no prompt, exit `1`. This is deliberate: an unattended apply is
  precisely the case with no one there to answer a prompt safely.
- **Interactive** (a human at a real TTY) gets a `y/N` confirmation naming the
  live canon it would overwrite; anything but an explicit `y`/`yes` cancels with
  nothing applied.

Pass `--force` to skip only the optimistic baseline check and overwrite
deliberately without either prompt — the explicit escape hatch for scripts that
intend to overwrite. `--force` never bypasses declaration validation or
`registry_content_mismatch`: a handler whose referenced live property type
differs from the bundle is still rejected before any write, because force does
not make that handler type-safe.
`--force` and `--baseline` together are rejected
(`CONTRADICTORY_BASELINE_INTENT`): pinning a baseline and overriding the pin in
the same call is contradictory intent — pick one.
...
## Go bilingual: locales, `@t:` keys, and `i18n/<locale>.json`

An app renders in two languages with three declarative moves, all living in
the manifest bundle you just exported:

1. **Declare support on the App** — `supported_locales: ["en", "da"]` +
   `default_locale: "en"` (must be a member). Two or more locales make the
   app renderable in the viewer's language preference — a tri-state Language
   row in **Account Settings** (English / Dansk / Organization default),
   account-wide and not per-app chrome — falling back to org/platform default
   where the app doesn't support the pick. Absent or a single entry ⇒
   monolingual, zero overhead.
2. **Write display text as message keys** — in an App with two or more
   `supported_locales`, every reachable display string must be
   `@t:<your.key>`, not a literal: property `label`, enum `valueLabels`,
   `display.entityKind`/`entityKindPlural`, state `label`s, action
   `presentation` (label/confirm/success/…), view `title`s, form fields, email
   templates. `catalog validate` has no generic display-literal advisory
   channel. A brand or
   acronym that should stay identical is still declared as a key with the same
   value in every locale. Stored verbatim, resolved per viewer at render.
   (The parser escape `@@t:foo` renders `@t:foo`.) Identifiers and enum **values** stay
   English `snake_case` in every locale — they are the API contract.
3. **Ship the words in the bundle** — one `i18n/<locale>.json` per declared
   locale (`{ "locale": "da", "managed_by": "tenant", "source_ref": null,
   "messages": { "customer.field.name": "Navn", … } }`), including your
   default locale. There is no organic verb for message files — they exist
   only in the bundle.

**The gate:** once any App declares `supported_locales`, `catalog.apply` /
`catalog validate` fail the bundle on `missing_message_key` (a referenced key
absent from a declared locale — the error names both), `missing_locale_bundle`
(a declared locale with no `i18n/<locale>.json`), and
`default_locale_not_supported`. For a multilingual App, keep every reachable
display string as an `@t:` key; the locale completeness checks remain the
build-time guard. Translation drift is a build error, not a runtime surprise.

**Bundle mode only:** organically invoking `apps.declare` with more than one
`supported_locales` entry against a live org is **refused at declare time**
(`VALIDATION`, `multi_locale_requires_bundle`) — organic authoring has no way
to supply the per-locale message bundles a multilingual app requires. Go
bilingual in bundle mode: export → key the strings + author `i18n/*.json` +
declare the locales in one edit → `validate` → `apply`.

In Danish, declared labels are **mandatory**, not nice-to-have: the undeclared
fallback humanizes the English identifier (`customer_name` → "Customer Name",
plural by English rules), which cannot be translated — the source text *is*
the identifier. Full chapter, the per-surface locale chains (web member, email
recipient, PDF envelope, portal signer), and a worked bundle in
[bilingual-apps.md](resources/bilingual-apps.md).
...
- [resources/bilingual-apps.md](resources/bilingual-apps.md) — go bilingual: `supported_locales`, `@t:` message keys, the same-value key escape for brands/acronyms, `i18n/<locale>.json`, the apply-gate errors, per-surface locale chains, and the bundle-mode-first authoring order
