...
>
> One class of `VALIDATION` failure is exempt from that decoration: the
> declare-time access-policy check (`invalid_access_policy`, see the base
> skill's *error codes* reference) runs before any sub-step write, so nothing
> is durable yet and re-applying converges nothing. Its `details.apply_state`
> is `"pre_write"` instead, and the message carries no re-apply-to-converge
> note — just the real problem to fix. Mirrors `STALE_CANON`'s pre-write
> posture above.
...
canon catalog apply ./my-app-canon --force              # deliberate overwrite, no check
```
...
Pass `--force` to skip the check and overwrite deliberately without either
prompt — the explicit escape hatch for scripts that intend to overwrite.
...
2. **Write display text as message keys** — any declared display string may
   be `@t:<your.key>` instead of a literal: property `label`, enum
   `valueLabels`, `display.entityKind`/`entityKindPlural`, state `label`s,
   action `presentation` (label/confirm/success/…), view `title`s, form
   fields, email templates. Stored verbatim, resolved per viewer at render.
   (Literal text that really starts with `@t:` is escaped by doubling:
   `@@t:foo` renders as `@t:foo`.) Identifiers and enum **values** stay
...
`default_locale_not_supported`. Translation drift is a build error, not a
runtime surprise.
...
- [resources/bilingual-apps.md](resources/bilingual-apps.md) — go bilingual: `supported_locales`, `@t:` message keys + the `@@t:` escape, `i18n/<locale>.json`, the apply-gate errors, per-surface locale chains, and the bundle-mode-first authoring order
