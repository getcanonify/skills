...
defect even when yours works (`C13-primitive-dup`). So this section means two
things, not one: *what has my org already built?* **and** *what does the
platform already ship?*
...
**Input shape:** `{ name: string, description?: string, confidential?: boolean, idPrefix?: string, properties?: {...}, links?: {...}, defaults?: {...}, display?: { title?, subtitle?, formFields?, primaryProperties?, searchable?, defaultFilter?, statusProperty?, actions? }, browsable?: boolean, rollups?: {...}, parts_frozen_when?: {...} }`
...
| `metric` | One scalar KPI tile (an aggregate without `groupBy`) — `display: "hero"`/`"gauge"`/`"bar"`, `goodDirection`, and a `trend` sparkline are optional presentation knobs |
...
work, and the coherence bar (`C5`–`C9`) you clear before calling anything done.
...
2. **Write display text as message keys** — in an App with two or more
   ...
   templates. `catalog validate` reports
   `elegance_untranslatable_display_literal`; `--strict` makes that advisory
   fail validation, while `catalog.apply` never blocks on elegance. A brand or
   ...
   (The parser escape `@@t:foo` renders `@t:foo`, but does not bypass the
   multilingual display-literal advisory.) Identifiers and enum **values** stay
...
`default_locale_not_supported`. For a multilingual App, they also report the
advisory `elegance_untranslatable_display_literal` for each reachable literal
display site; `catalog validate --strict` makes advisories blocking, while
`catalog.apply` strips all `elegance_*` findings from its gate. Translation drift
