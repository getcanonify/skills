...
- **`name`** — `<object>.<verb>`, lowercase, dot-separated (regex
  ...
  and may not be `platform` or start with `platform.` — that namespace is
  reserved for the control-plane audit trail (see the base skill's
  *declaring-actions* reference).
