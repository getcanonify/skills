...
| **Input** | `{ name: string, description?: string, confidential?: boolean, idPrefix?: string, properties?: {...}, links?: {...}, defaults?: {...}, display?: { title?, subtitle?, formFields?, primaryProperties?, searchable?, defaultFilter?, statusProperty?, actions? }, browsable?: boolean, rollups?: {...}, parts_frozen_when?: {...}, expectedHash?: string, force?: boolean }` |
...
**Re-refining an already-refined type is guarded.** Omit both `expectedHash`
and `force` on a type that already has a prior refinement and the call is
rejected before any write (`CONFLICT/HASH_REQUIRED`) rather than silently
overwriting a teammate's change — the FIRST refinement of a type has nothing
to conflict with, so this never fires on a fresh type. See
...
for the full guard contract (mirrors `actions.update`'s `expectedHash`/`force`).
...
## Object-level display and sensitivity metadata

The five fields below are authorable in the same refinement call. Each is
optional for compatibility: when it is omitted, the existing renderer
fallback remains in force. An explicit value is the app's contract and is
preserved by `catalog.export` → `catalog.apply`.

### Filter-bar columns — `display.searchable`

`display.searchable` is the ordered list of properties the default list page
offers in its filter bar:
...
When omitted, the fallback is the string- and enum-typed properties among the
inferred `primaryProperties` (including a string foreign-key column, if it is
among those columns). This is display metadata, not a permission grant or a
replacement for the server's enforced searchable-column set.

### Status pill — `display.statusProperty`

Declare the enum property whose values render as the default list/detail status
pill:

```yaml
display:
  statusProperty: lifecycle_stage
```

When omitted, the fallback is the first enum property whose name contains
`status` or `stage`; when none matches, no status pill is shown. The declared
property should be an enum in the ObjectType.

### Browse/picker default — `display.defaultFilter`

`display.defaultFilter` supplies a renderer-owned default for browse and
...
When omitted, no default filter is added. This is explicitly **not policy**:
callers may still use typed reads to request inactive or historical rows, and
an explicit caller filter is not a security bypass or a replacement for the
runtime authorization gate.

### Sensitive type marker — `confidential`

Set `confidential: true` when the ObjectType contains sensitive data:

```yaml
confidential: true
```

When omitted (or false), ordinary access/audit treatment remains in force. The
marker does not grant access itself. In the production deny-by-default policy,
an ungranted read or action is still rejected with `FORBIDDEN` and its audit
row has `status: denied`; confidentiality does not change that denial message
or code. A granted mutation targeting a confidential type is instead audited
with `status: sensitive`, which lets reviewers find sensitive writes without
cross-referencing the catalog. Granted reads currently use the reads-denials-
only policy and therefore do not persist a success audit row.

### Generated-id prefix — `idPrefix`

Set the prefix used by generated ids on typed inserts:

```yaml
idPrefix: lead
```

When omitted, the existing compatibility fallback remains: generated
auto-CRUD ObjectTypes use `rec_`, while lower-level typed writes fall back to
`Prefix.fromTable(table)`. The prefix is effectively **write-once**: changing
the declaration later changes the prefix for new generated ids only; existing
rows keep the prefix already written into their ids. Choose it before creating
rows when stable id prefixes matter.

## Property display label — `label`

Any property may declare an optional `label`: the **human name the UI renders for
the field** — table header, detail field, action-form field, filter control.
...
Without it, every label is **derived** from the property name
(`default_price` → "Default Price"). That default is fine for an English app and
is unchanged when you declare nothing — but it means the app can only ever be as
human as its column names, and it cannot be translated at all, because the
displayed text *is* the identifier. `label` is where you say the name yourself.

The label is **display-only**: the property NAME remains the API contract on
REST/CLI/MCP, and filtering/sorting/`?where=` are untouched — so you can rewrite a
label freely without breaking a saved filter or an integration. It is
**type-independent** (unlike `values`/`valueColors`/`currency`, any property type
may carry one) and **inherit-when-omitted**: a later refinement that re-states the
property without `label` keeps the prior one, and — because the label is not an
enum-only attribute — it also survives a change of `type`. A blank/whitespace
label fails `catalog.apply` with `invalid_property_label`; omit the field
entirely to fall back to humanisation. `label` is opaque display text — it may
be a `@t:` message key instead of a literal (see the bilingual chapter).

> `label` is the field's **name**; `description` (and `fieldDescriptions`) is
> **prose about** the field. They are different things — a description is a
> sentence and is never used as a header.

### Property descriptions — `properties.<name>.description`

Any mapped or derived property may carry optional prose about the field. It is
stored on the property and projected through the ObjectType's
`fieldDescriptions` map in `schema_describe` and discovery; it is not the
display label used for a column heading.
...
properties:
  status:
    ...
    type: enum
    values: [open, closed]
    description: The current support workflow state.
```
...
humanised token (`green` → `Green`) for compatibility, and the catalog audit
surfaces that missing label. Same **inherit-when-omitted** rule as `valueColors`;
a key not in `values`, or a blank label, fails `catalog.apply` with
`invalid_enum_label`. Each value's text is opaque display text — it may be a
`@t:` message key instead of a literal (see the bilingual chapter). The base
`canonify` skill's
[declaring-objecttypes › *Enum value labels*](../../canonify/resources/declaring-objecttypes.md)
carries the full semantics. Any enum value without a non-empty `valueLabels`
entry draws the `elegance_missing_value_labels` advisory. A bound StateMachine's
non-empty `StateMeta.label` is an equivalent label source.
...
`on_delete` applies only to target-side ownership links (`fk_on: target` or
omitted), where the target carries the child FK. A source-side belongs-to link
(`fk_on: source`) is navigational and must omit `on_delete`; declaration and
catalog validation reject that inert configuration. Put the policy on the
reverse target-side link.

### `on_delete` policies

For target-side links, honoured by **every** delete path, not just declared
handlers:
...
### `defaults` — per-verb auto-CRUD opt-out

Set a verb to `false` to remove its generated default ActionType from every
surface. The ordinary CRUD flags (`create`, `read`, `update`, `delete`, and
`list`) apply to every ObjectType. A compound root can also set
`replace: false` to suppress its generated whole-cluster replacement action:

```yaml
input:
  name: invoice
  defaults:
    replace: false   # invoice.replace is absent from REST, CLI, and MCP
```

`replace:false` is valid on a type with no `part_of` children and is simply a
no-op there, because that type never generated a `replace` ActionType. This
keeps the refinement forward-compatible if the type later becomes compound.

## Display & titles — every object needs a human title

The `display` block tells the renderer (and search, and action forms) how to
show a record to a **human**. It carries `title`, `subtitle`, `entityKind`,
`entityKindPlural`, `formFields`, `primaryProperties`, and `actions`
(per-action presentation, below) — all optional, all additive. Its keystone is `title`: **every ObjectType must
...
### Type display name — `entityKind` / `entityKindPlural`

The **type itself** has a display name too. All the generated chrome — the
list heading, the "Create …" button, the Browse-data sidebar entry, empty
states ("No customers yet"), detail linked-section headers — derives it from
the machine identifier by default (`customer` → "Customer" / "Customers").
Declare `display.entityKind` (singular) and `display.entityKindPlural`
(plural) to override both forms:
...
input:
  ...
  display:
    entityKind: Kunde
    entityKindPlural: Kunder
```

Declare **both** when you declare either. The renderer never pluralizes a
declared name for you — the auto-plural is English-only (`kunde` would come
out "kundes", not "kunder"), so a plural context with no declared
`entityKindPlural` falls back to humanizing the identifier instead. Both
fields are opaque display text (they may be `@t:` message keys — see the
bilingual chapter); a blank/whitespace value fails with `invalid_entity_kind`.
...
| Re-refine with no `expectedHash`/`force` | `CONFLICT/HASH_REQUIRED` | read the current `declaration_hash` and re-submit, or pass `force: true` |
