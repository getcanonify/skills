...
| **Input** | `{ name: string, description?: string, confidential?: boolean, idPrefix?: string, properties?: {...}, links?: {...}, defaults?: {...}, display?: { title?, subtitle?, formFields?, primaryProperties?, searchable?, defaultFilter?, statusProperty?, actions? }, browsable?: boolean, rollups?: {...}, parts_frozen_when?: {...}, temporal?: { from, to, key }, expectedHash?: string, force?: boolean }` |
...
humanised token (`green` → `Green`) for compatibility. Same
**inherit-when-omitted** rule as `valueColors`;
...
carries the full semantics. Add labels when the displayed vocabulary matters; a
bound StateMachine's non-empty `StateMeta.label` is an equivalent label source.
