...
2. **Add `valueLabels` on those same enums.** A colored chip that still shows the
   ...
   touches filtering (queries still key off the raw value). Any enum value
   without a non-empty label earns the `elegance_missing_value_labels` advisory;
   a bound StateMachine's `StateMeta.label` is an equivalent label source.
...
they just render *flat*. Add `"compare": { "created_at": "$now-30d.." }` and the
tile grows a delta badge ("+12%" vs the prior 30 days); pair it with
`"goodDirection": "up"` (or `"down"` for a metric like churn, where a rise is
bad news) and that badge tones green/red instead of staying neutral — the same
polarity also ramps a gauge's fill through warning/danger as it nears `target`.
For the one number a dashboard should lead with, `"display": "hero"` renders it
at headline scale (≥48px) — use it on exactly one tile per view, never more.
And `"trend": { "bucket": "month", "periods": 12 }` draws a twelve-point
sparkline under the figure, so "up 12%" reads with the shape behind it instead
of as a bare claim.
