...
platform's 2026-07 ViewSpec craft audit.
...
2. **Add `valueLabels` on those same enums.** A colored chip that still shows the
   ...
   touches filtering (queries still key off the raw value). Add a non-empty
   label for every value whose displayed vocabulary matters; a bound
   StateMachine's `StateMeta.label` is an equivalent label source.
3. **Group and emphasize your actions.** More than a couple of custom actions
   ...
   a clear inline CTA. This keeps a large action surface readable.
4. **Give every `detail` a `header`.** `{ title, subtitle, status }`
...
7. **Curate columns.** Pick 4–6 that matter, set `format`, and use semantic
   `emphasize` rules on the one cell that carries the signal. A 12-column dump reads
...
The elevated-detail pattern: a **header band** (title/subtitle/status), a
...
A list an operator can *scan*, not just read: curated columns with `format`, a
value column with semantic `emphasize` rules for the row that matters, a status
...
```json
{
  ...
  "spec": {
    ...
    "columns": [
      { "ref": "name", "label": "Deal", "sortable": true },
      "stage",
      {
        "ref": "amount",
        "label": "Value",
        "format": "currency",
        "sortable": true,
        "emphasize": [{ "when": [{ "field": "stage", "op": "eq", "value": "won" }], "level": "good" }]
      },
      { "ref": "close_date", "label": "Close", "format": "date" },
      "owner"
    ],
    "presets": [
...
and grey text in the detail. `emphasize` puts a semantic highlight on
...
> **Column formatting.** `format` is `currency | percent | date` and is checked
> against the property type at declare time. Use `emphasize` for semantic status
> treatment; columns share the available width automatically.
...
seats used vs. licensed, quota attainment — a scalar tile beside a grouped
`table` is the canonical dashboard
([viewspec-analytics › *KPI dashboard composite*](../../canonify-viewspecs/resources/viewspec-analytics.md)).

Two more knobs are easy to miss because nothing renders wrong without them —
they just render *flat*. Give the tile a time spine (`"groupBy": "created_at@month"`
on its source) and `"window": { "last": 12 }` and it draws a twelve-point
sparkline under the figure, which becomes the newest month; add
`"compare": { "offset": { "period": "year", "periods": 1 } }` and the tile grows
a delta badge ("+12%" vs the same month last year) — the same `compare` /
`window` a `chart` and a grouped `table` carry, read in one request.
...
- the platform's 2026-07 ViewSpec craft audit — the findings these moves
  answer.
