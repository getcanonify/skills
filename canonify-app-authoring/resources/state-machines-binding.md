...
| **Input** | `{ kind: 'state_machine', name, objectType, property, initial, states, transitions, on_enter?, on_exit?, expectedHash?, force? }` |
| **Envelope** | `{ data: { name, registered: true }, audit_event_id, approval_status: 'not_required' }` |

**Re-registering an already-registered machine is guarded.** Omit both
`expectedHash` and `force` on a `name` that's already registered and the call
is rejected before any write (`CONFLICT/HASH_REQUIRED`) — the FIRST
registration has nothing to conflict with, so this never fires on a fresh
machine. See
[declaring-state-machines.md](../../canonify/resources/declaring-state-machines.md#optimistic-concurrency--expectedhash--force-on-re-register)
for the full guard contract.
...
| Re-register with no `expectedHash`/`force` | `CONFLICT/HASH_REQUIRED` | read the current `declaration_hash` and re-submit, or pass `force: true` |
