...
4. **A multilingual App cannot carry fixed display copy.** When an App declares
   ...
   from its nav must be an `@t:` key. The locale completeness checks are the
   build-time guard; there is no separate display-literal advisory channel.
...
(`@@t:foo`) remains a way to represent a literal that starts with `@t:`; use a
same-value message key at a user-facing display site when the text must be
...
completeness checks runs. A multilingual App must still use `@t:` keys for all
reachable display strings; there is no separate display-literal advisory
channel.
...
The locale contract covers the fixed display sites in those reachable
