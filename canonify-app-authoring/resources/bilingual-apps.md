...
Any **declared display string** may be a message key instead of literal text:
the value starts with the sigil `@t:` and the rest is a key you invent
(`@t:customer.field.name`). The key is stored **verbatim** — it is resolved
against the viewer's locale at render, never baked to one language at declare
time.
...
**The escape hatch.** Literal text that genuinely starts with `@t:` is
authored with a doubled sigil — `@@t:foo` renders as the literal `@t:foo`,
and is never treated as a key (the apply gate never demands a translation for
it). Doubling the leading `@` is the one and only escape.
...
three completeness checks. When no App declares locales, none of this runs.
