...
A closed set of ten variants. Every variant accepts an optional `title`
...
| `metric` | one scalar KPI tile (aggregate without `groupBy`) | `source`, `label`, `format?`, `display?`, `target?`, `goodDirection?`, `compare?`, `trend?`, `title?` |
| `chart` | a bar or line chart (aggregate **with** `groupBy`) | `source`, `kind?`, `stacked?`, `format?`, `title?` |
...
`metric`'s `display` (`"scalar"` default, `"gauge"`, `"bar"`, `"hero"`) picks
the presentation; `goodDirection` (`"up"` \| `"down"`) tones the delta badge
and the meter fill; `trend: { property?, bucket, periods }` draws a sparkline.
