...
description: Use view_specs.declare to shape how the app renders — table, detail, panel, button, composite, metric, chart, file, board, and queue. A ViewSpec shadows the renderer auto-default for an ObjectType and is validated against the live registry at declare time.
---
...
For paths, expand, measures, lookups, and reader filters, use the canonical
`canonify-viewspecs` references below. The app-authoring skill owns the
authoring loop; the ViewSpec skill owns the complete closed grammar:

- [binding and source paths](../../canonify-viewspecs/resources/viewspec-binding-and-sources.md)
- [expand, measures, lookups, and the PortionPlan](../../canonify-viewspecs/resources/viewspec-analytics.md)
- [composition filters and `$context`](../../canonify-viewspecs/resources/viewspec-composition-patterns.md)
...
A closed set of ten variants. Every variant accepts an optional `title` (the
block's name, painted once in its own header; a `composite` paints it as a
section header its children inherit) and an optional `showWhen` predicate
...
| `metric` | one KPI tile (a scalar aggregate, or a time-bucketed `groupBy` spine + `window` + `compare`) | `source`, `label`, `format?`, `compare?`, `window?`, `title?` |
| `chart` | a bar or line chart (aggregate **with** `groupBy`) | `source`, `kind?`, `stacked?`, `format?`, `compare?`, `window?`, `title?` |
...
`metric`, `chart` and a grouped `table` share ONE aggregate envelope: `source`
(with an optional time-bucketed `groupBy` spine such as `"created_at@month"`),
`window: { last: N }` (the newest N buckets) and `compare: { offset?, rolling? }`
(same-measure comparison layers). On a metric the newest bucket is the figure,
its compare layer the delta badge, and the window the sparkline — one read.
Full field reference in
[viewspec-analytics](../../canonify-viewspecs/resources/viewspec-analytics.md).

**`composite` layout axis** — `layout` is one of `"stack"` (default, vertical),
`"grid"` (responsive columns; children may be `{ span: 2 | 4, view: <spec> }`
...
`title` is a separate caption painted above the button.
...
- `deals.sum:amount` — `sum`/`avg`/`max` take a `:property`
...
| A column names a dot-traversal ref (`"customer.tier"`) with `sortable: true` | `unsupported_sortable_column_ref` | relation-path sorting is not supported by design; denormalize the value or sort by a local source ObjectType property |
| a categorical `groupBy` (or `seriesBy`) on a `metric` source | `invalid_metric_group_by` | a metric renders one figure — use a time-bucketed spine (`"created_at@month"`) for a sparkline, or declare a `chart` / grouped `table` |
| `compare` / `window` without a time-bucketed `groupBy` spine | `invalid_aggregate_compare` / `invalid_aggregate_window` | the envelope needs a `@bucket` spine on `metric`, `chart` and grouped `table` alike |
| no `groupBy` on a `chart` source | `invalid_chart_group_by` | a chart needs a dimension axis — add `groupBy`, or use `metric` for a single scalar |
